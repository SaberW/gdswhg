        ...
        $.extend($.fn.validatebox.defaults.rules, {
            bmEndTime: {
                validator: function(value, param){
                    ...
                    //var sdVal_startDate = $('#'+param[1]).datebox('getValue');//活动开始时间
                    ...
                    //var d3 = WhgComm.parseDate(sdVal_startDate);
                    //return d2.getTime()>d1.getTime() && d3.getTime() > d2.getTime();
...
<form id="whgff" class="whgff" method="post">
    ...
    <div class="whgff-row">
		...
		<div class="whgff-row-input">
			<input class="easyui-textbox" name="host" value="${act.host }"
				style="width: 500px; height: 32px" data-options="required:false,validType:['length[0,50]']" />
		</div>
	</div>
	<div class="whgff-row">
	...
	<div class="whgff-row">
		...
		<div class="whgff-row-input">
			...
			<input type="text" class="easyui-datetimebox"style="width: 240px; height: 32px;" id="enterendtime" name="enterendtime"  value="<fmt:formatDate value='${act.enterendtime}' pattern="yyyy-MM-dd HH:mm:ss"></fmt:formatDate>" data-options="editable:false,required:true,prompt:'请选择',validType:'bmEndTime[\'enterstrtime\',\'starttime\']'" />
		</div>
	</div>
    ...
    function  setSeat(whgSeat) {
        ...
        if("3" == sellticket){
            ...
            if(myWhgSeat != null && myWhgSeat != ''){
                ...
                var roomVal = $("#roomid").combobox("getValue");
                var roomData = $("#roomid").combobox("getData");
                for(var room in roomData){
                    if (room.id == roomVal){
                        rowNum = room.seatrows;
                        colNum = room.seatcols;
                        seatjson = rec.seatjson? JSON.parse(rec.seatjson) : [];
                    }
                }
            }
