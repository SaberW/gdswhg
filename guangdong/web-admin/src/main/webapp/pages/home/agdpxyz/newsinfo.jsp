...
<script src="${basePath }/static/assets/js/public/public.js"></script>
	...
	function  showInfoList(data) {
		...
		for(var i in data.rows){
			...
			infoHtml+='<li><a href="${basePath }/agdpxyz/newsinfo?id='+model.clnfid+'">'+model.clnftltle+'</a><p class="time">'+moment(model.clnfcrttime).format("YYYY-MM-DD")+'</p></li>';
			}
		...
		<div class="public-left-main">
			<div class="noticeInfo-main">
				...
				<div class="otherInfo">
					...
					<span class="key">关键字 :<span id="showkey">${wh_zx_colinfo.clnfkey }</span>
					<span class="time">时间 :<span id="showtime"><fmt:formatDate value="${wh_zx_colinfo.clnfcrttime}"
					...
					<div class="file-download-cont">
						<ul>
							<c:forEach items="${loadlist}" var="loadlists" varStatus="s">
								<li>
									<a href="${basePath }/whtools/downFile?filePath=${loadlists.uplink}" class="js-urlencode"><i></i>${loadlists.upname}
									</a>
								</li>
							</c:forEach>
						</ul>
