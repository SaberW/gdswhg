					   ...
					   <c:forEach items="${rows }" var="row" varStatus="s">
			                <li>
			                    ...
			                    <div class="detail">
                                    ...
                                    <p class="time">报名状态 :<span>
                                        <c:choose>
                                            <c:when test="${row.isbm eq 1}">
                                                <c:if test="${row.ordernum ge row.maxnumber }">报名人数已满</c:if>
                                                <c:if test="${row.ordernum lt row.maxnumber }">报名中</c:if>
                                            </c:when>
                                            <c:otherwise>
                                                <fmt:parseDate value="${row.enrollstarttime}" var="stime" pattern="yyyy-MM-dd HH:mm:ss"></fmt:parseDate>
                                                <fmt:parseDate value="${now}" var="ntime" pattern="yyyy-MM-dd HH:mm:ss"></fmt:parseDate>
                                                <c:if test="${stime gt ntime}">报名未开始</c:if>
                                                <c:if test="${stime lt ntime}">报名已结束</c:if>
                                            </c:otherwise>
                                        </c:choose>
                                    </span></p>
                                    <p class="time">人数 :<span>${row.ordernum} / ${row.maxnumber}</span> </p>
