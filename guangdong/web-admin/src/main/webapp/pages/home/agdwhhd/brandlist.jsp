...
function loadData(page, rows){
	...
	$.ajax({
		type: 'post',
		url: '${apiRoot}/api/outer/activity/hdpplist',
		data: _param,		
...
function showData(data){
	...
	if(data.length > 0){    
		for(var i=0 ;i<data.length; i++){
			var row = data[i];
			var pic = "${apiStaticPath}"+WhgComm.getImg300_200(row.picture)+"" ;
			var _html = '';
			 _html+='<li>                                                                                                 '
			 _html+='    <a href="${apiWbgx}${apiSite}/pp/detail?id='+row.id+'" target="_blank">               '
			 _html+='        <div class="img" style="background-image: url('+pic+')">                                     '
			 _html+='            <div class="mask"></div>                                                                 '
			 _html+='        </div>                                                                                       '
			 _html+='    </a>                                                                                             '
			 _html+='    <div class="detail">                                                                             '
			 _html+='        <a href="${apiWbgx}${apiSite}/pp/detail?id='+row.id+'" target="_blank"><h2>'+row.name+' </h2></a>                            '
			 _html+='        <p>'+row.introduction+'</p>                                                                  '
			 _html+='        <a class="enter-zt" href="${apiWbgx}${apiSite}/pp/detail?id='+row.id+'" target="_blank">进入专题</a>'
			 _html+='    </div>                                                                                           '
			 _html+='</li>                                                                                               '
...
$(function(){
	...
	$.extend(_param,{cultsite:"${apiSite}"});
	loadData();
})
</script>
