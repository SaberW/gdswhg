...
<div class="main clearfix">
	...
	<div class="rightPanel">
   		<ul class="commBtn clearfix">
    		   ...
    		   &lt;%&ndash;<li cmreftyp="3"><a href="javascript:void(0)">场地收藏</a></li>&ndash;%&gt;
		...
		<div class="safe-row safe-row-1 clearfix">
			<a class="wbgx-link" ></a>
			<div class="ico"></div>
			<div class="safeTt">外部供需收藏</div>
			<div class="safeState">查看活动、场馆及活动室的收藏信息</div>
			<div class="setUp_none"></div>
		</div>
		<div class="safe-row safe-row-4 clearfix">
			<a class="zxpx-link" ></a>
			<div class="ico"></div>
			<div class="safeTt">培训收藏</div>
			<div class="safeState">查看培训的收藏信息</div>
			<div class="setUp_none"></div>
		</div>
...
<script>
    ...
    $(function(){
        ...
        if (/\/$/.test(apiZxpx)){
            apiZxpx = apiZxpx.replace(/\/$/, '');
        }

        var linkUserParams = '?phoneNum=${wbgx_sessUserKey.mobile}';
        var _wbgxLink = apiWbgx+'/'+apiSite+'/center/favorite'+linkUserParams;
        $("a.wbgx-link").attr({ "href": _wbgxLink, "target":"_blank" });

        var _zxpxLink = apiZxpx+'/'+apiSite+'/usercenter/myFav'+linkUserParams;
        $("a.zxpx-link").attr({ "href": _zxpxLink, "target":"_blank" });
	});
