...
function loadData(page, rows){
	...
	$.ajax({
		...
		data:{rmuid:"${wbgx_sessUserKey.id}",rmreftyp:rmreftyp,page:page,rows:rows},
...
function showData(data, commentReMap){
	...
	if(data.length > 0){
		for(var i=0 ;i<data.length; i++){
			...
			_html += '			</p>';
			for(var j=0; j<commentReMap.length;j++){
                if (commentReMap[j].rmrefid != row.rmid){ continue }
				_html += '			<p class="reComm">';
				_html += '				<span>'+(commentReMap[j].account||"")+' :</span>'+commentReMap[j].rmcontent;
				_html += '			</p>';
			}
