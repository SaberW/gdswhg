...
$(document).ready(function(e){
	...
	if($(".tt").length>0){
		$.ajax({
			type : "POST",
			url : basePath+"/center/hasSessPhone",
				...
				if(data.success == "1"){
				    ...
				    */
					...
					$("#sure").click(function(){
						...
						$.ajax({
							type : "POST",
							url : basePath+"/center/modifyPhone",
							data : {phone : phone},
							success : function(data){
								if(data.success == "0"){
									rongDialog({
    ...
    */
	...
	function _validPrePhone(preMsgPhone,inputEl,sendMsg){
		...
		$.ajax({
			type:"POST",
			async:false,
			url:basePath+"/center/isPrePhone",
