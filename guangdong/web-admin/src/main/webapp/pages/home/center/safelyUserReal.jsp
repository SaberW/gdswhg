...
var tool = (function(){
	//会话用户是否已实名
	var isrealname = "${wbgx_sessUserKey.isrealname}" ||0;
	//会话用户ID
	var id = "${wbgx_sessUserKey.id}";
	//图片上传的处理URL
	...
	function showState(data){
		if (!data){
			data = new Object();
			data.name = "${wbgx_sessUserKey.name}";
			data.idcard = "${wbgx_sessUserKey.idcard}";
			data.idcardface = "${wbgx_sessUserKey.idcardface}";
			data.idcardback = "${wbgx_sessUserKey.idcardback}";
		...
		if (data.name && data.idcard && data.idcardface && data.idcardback){
			...
			if (data.isrealname == 2){
			    $('#checkMsg').show().find('div').text('${wbgx_sessUserKey.checkmsg}');
	...
	function submitInfo(){
		...
		$(this).parents("div.rightPanel").find("input[name]").each(function(){
			...
			if(name != "sex"){
				...
				args[name] = $("input[name=sex]:checked").val();
			}
	...
	function init(){
		...
		var _phone = "${wbgx_sessUserKey.mobile}";
		if(_phone == null || "" == _phone ){
			rongDialog({ type : true, title : "您还未绑定手机,请先绑定手机", time : 2*1000 ,url:"${basePath}/center/safely-phone"});
		}
...
<div class="main clearfix">
    ...
    <div class="rightPanel">
        ...
        <dl class="clearfix">
          ...
          <dd class="float-left">
            <input name="name" value="${wbgx_sessUserKey.name }" class="in-txt" placeholder="真实姓名">
            <em class="err"></em>
          </dd>
        </dl>
		<dl class="clearfix">
			...
			<dd class="float-left">
				<div class="">
					<input type="radio" value="1" name="sex" ${empty wbgx_sessUserKey.sex ? "checked=''" :""} ${not empty wbgx_sessUserKey.sex and wbgx_sessUserKey.sex == 1 ? "checked=''":""} id="r1"> <span>男</span>
					<input type="radio" value="0" name="sex" ${not empty wbgx_sessUserKey.sex and wbgx_sessUserKey.sex == 0 ? "checked=''":""} id="r2"> <span>女</span>
				</div>
			</dd>
		</dl>
        <dl class="clearfix">
          ...
          <dd class="float-left">
            <input name="idcard" value="${wbgx_sessUserKey.idcard }" class="in-txt" placeholder="请输入18位正确的身份证号码"/>
            <em class="err"></em>
          </dd>
        </dl>
        <dl class="clearfix">
            ...
            <dd class="float-left">
            <c:choose>
				<c:when test="${not empty wbgx_sessUserKey.idcardface }">
            	<a href="javascript:void(0)" class="card cardA isUploaded"><span>
            		<img alt="" src="${imgServerAddr }/${wbgx_sessUserKey.idcardface}" width="310" height="160" />
            	</span></a>
				</c:when>
				<c:otherwise>
        ...
        <dl class="clearfix">
            ...
            <dd class="float-left">
            <c:choose>
				<c:when test="${not empty wbgx_sessUserKey.idcardback }">
            	<a href="javascript:void(0)" class="card cardB isUploaded"><span>
            		<img alt="" src="${imgServerAddr }/${wbgx_sessUserKey.idcardback}" width="310" height="160" />
            	</span></a>
				</c:when>
				<c:otherwise>
