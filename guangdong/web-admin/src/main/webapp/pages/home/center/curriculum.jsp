...
<script>
    ...
    $(function(){
        ...
        $.get(apiRoot+'api/outer/comm/backUser', {userphone: sessUserPhone }, function(data){
            ...
            refUserId = data.data;
            _loadData('0',null,1,10);
        }, 'json');
    });
	/** 备份旧的加载数据方法
	 * @param type 0-报名中;1-审核中;2-已报名
	 */
	function _loadData_old(type, input, page, rows){
    ...
    function _loadData(type, input, page, pageSize){
	    if (!refUserId || refUserId == '') {return;}
	    if (input){
        	$(input).addClass("active").siblings().removeClass('active');
		}
        var page = page || 1;//分页加载第一页
        var pageSize = pageSize || 10;//每页10条记录
        //请求服务器数据 api/px/mytra/list
        $.ajax({
            type: "POST",
            url: apiRoot+'api/px/mytra/list',
			data: {
                page: page,
				pageSize: pageSize,
                userid: refUserId,
            ...
            success: function(data){
                ...
                //加载分页工具栏
                genPagging('artPagging', page, pageSize, total, function(page, pageSize){
                    _loadData(type, input, page, pageSize);
                });
            }
        });
    }
	 ...
	 * @returns
	 */
	function showList(dataRows, type, data){
        //移除原有数据
		...
		if(dataRows.length > 0){
			for(var i=0; i<dataRows.length; i++){
				...
				var crttime =  new Date(row.starttime);
				var nowTime = new Date(data.data.twodaytime);
				//alert(nowTime.getTime() +"----"+row.crttime);
				...
				item.find(".title").attr("href",basePath+'/agdpxyz/traininfo?traid='+row.traid);

                item.find(".address").text(row.address);
				var _pxtimes = new Date(row.starttime).Format("yyyy-MM-dd")
					+ ' 至 ' + new Date(row.endtime).Format("yyyy-MM-dd")
                item.find(".starttime").text(_pxtimes);
                item.find(".phone").text(row.phone);
                item.find(".orderid").text(row.id);

                //if(row.state == 1 && type == 1){
                if(row.state == 1 && type == 0){
                    item.find(".docancel").css("display","");
                    item.find(".docancel").attr("enrolId",row.id);
				}

				//if(row.state == 1 && type == 1 && nowTime>crttime){
				if(row.state == 1 && type == 0 && nowTime>crttime){
					item.find(".docancel").css("display","none");
					item.find(".docancel").attr("enrolId",row.id);
				}
				if(row.state == 2 ){
					item.find(".docancel").css("display","");
        ...
        $('.docancel').on('click', function(){
			...
			if(state == 2){
				rongAlertDialog({ title: "提示信息", desc : "您确定删除该培训报名吗？", closeBtn : false, icoType : 1 }, function(){
                    ...
                    var _url = apiRoot+"api/px/mytra/offenroll";
...
<div class="main clearfix">
	...
	<div class="rightPanel">
		<ul class="commBtn clearfix">
			...
			<li class="active" onclick="_loadData('0',this)"><a href="javascript:void(0)" >线下培训</a></li>
		</ul>
