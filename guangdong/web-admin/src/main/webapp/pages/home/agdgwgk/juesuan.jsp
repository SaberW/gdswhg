...
<head>
    ...
    <title>广东省文化馆-馆务公开-财政预决算</title>
    <script type="text/javascript">
        /** page onload event */
        $(function(){
            //加载分页工具栏
            genPagging('whgPagging', '${page}' || 1, 8, '${total}', function(page, rows){
                window.location.href = '${basePath}/agdgwgk/juesuan?page='+page+'&rows='+rows;
            });
        });
    </script>
</head>
    ...
    <div class="header-nav-bg">
        <div class="header-nav">
            ...
            <ul>
                ...
                <li class="active"><a href="${basePath }/agdgwgk/juesuan">财政预决算</a></li>
...
<!--资讯广告开始-->
<div class="public-information-ad" style="background:url(${basePath }/static/assets/img/museum/adBanner3.jpg) no-repeat 50% 50%">
    <div class="posi-fix">
        <div class="titleBg" style="background:url(${basePath }/static/assets/img/museum/title-bg-3.png) no-repeat 50% 50%;"></div>
    </div>
</div>
<!--资讯广告结束-END-->
        ...
        <c:choose>
            <c:when test="${rows != null && fn:length(rows) > 0}">
                <c:forEach items="${rows }" var="row" varStatus="s">
                    <li>
                        <i></i>
                        <h2><a href="${basePath }/agdgwgk/juesuaninfo?id=${row.clnfid}">${row.clnftltle }</a></h2>
