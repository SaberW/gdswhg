...
import com.creatoo.hn.mapper.home.CrtPxyzMapper;
...
public class PxyzService {
    ...
    @Autowired
    private CrtPxyzMapper crtPxyzMapper;
	...
	public Map<String, Object> paggingzxbm(Map<String, Object> param)throws Exception {
		...
		for (int i = 0; i < list.size(); i++) {
			if (list.get(i).getAddress()!=null && list.get(i).getAddress().length() > 20) {
				String address = list.get(i).getAddress().substring(0, 20);
				list.get(i).setAddress(address);
			}
            if(list.get(i).getAge() != null && !"".equals(list.get(i).getAge()) ){
                String[] age = list.get(i).getAge().split(",");
                list.get(i).setAge(age[0]+"-"+age[1]);
            }
		}*/
        List list = this.crtPxyzMapper.selectPxyzList(param);
        if (list!=null && list.size()>0){
            Date now = new Date();
            for(Object obj : list){
                Map ent = (Map) obj;
                if (ent.get("address")!=null && ((String) ent.get("address")).length()>20 ){
                    String address = (String) ent.get("address");
                    address = address.substring(0, 20);
                    ent.put("address", address);
                }
                if (ent.get("age")!=null && !((String)ent.get("age")).isEmpty() ){
                    String[] age = ((String)ent.get("age")).split(",");
                    if (age.length>1) {
                        ent.put("age", age[0] + "-" + age[1]);
                    }
                    if (age.length==1){
                        ent.put("age", age[0]);
                    }
                }
//                Date enrollstarttime = (Date) ent.get("enrollstarttime");
//                Date enrollendtime = (Date) ent.get("enrollendtime");
//                if (enrollstarttime !=null && enrollendtime!=null){
//                    if (enrollstarttime.getTime()> now.getTime()){
//                        ent.put("bmstate", "报名未开始");
//                    }
//                    if (enrollstarttime.getTime() <= now.getTime() && now.getTime()<= enrollendtime.getTime()){
//                        long notnum = (Long) ent.get("notnum");
//                        if (notnum<1){
//                            ent.put("bmstate", "报名数量已满");
//                        }else {
//                            ent.put("bmstate", "报名中");
//                        }
//                    }
//                    if (now.getTime() > enrollendtime.getTime()){
//                        ent.put("bmstate", "报名已结束");
//                    }
//                }
            }
        }
...
//		}
	...
	public int selCount(String traid) {
		...
		example.createCriteria().andEqualTo("traid",traid).andIn("state", Arrays.asList(1,4,5,6));
