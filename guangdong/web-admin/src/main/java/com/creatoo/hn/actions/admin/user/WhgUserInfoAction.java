...
public class WhgUserInfoAction {
    ...
    public ResponseBean srchList4p(HttpServletRequest request, WhUser_old user){
        ResponseBean res = new ResponseBean();
        try {
            PageInfo<WhUser_old> pageInfo = this.userService.t_srchList4p(request, user);
    ...
    public ResponseBean edit(HttpServletRequest request, WhUser_old user){
        ResponseBean res = new ResponseBean();
        try {
            userService.t_edit(user, (WhgSysUser) request.getSession().getAttribute("user"));
            try{
                WhUser_old myuser = this.userService.t_srchOne(user.getId());
