...
public class WhgTrainCourseAction {
    ...
    @ResponseBody
    public ResponseBean srchOne(String traid,String userid){
        ResponseBean rsb = new ResponseBean();
        rsb = this.whgTrainCourseService.t_srchOne(traid,userid);
        return rsb;
    }
     ...
     */
    @RequestMapping("/add")
    @WhgOPT(optType = EnumOptType.TRA, optDesc = {"新增课程"})
    public ResponseBean add(String traid, WhgTraCourse course, HttpSession session, HttpServletRequest request){
        ...
        try {
           ...
           WhgTra tra= whgTrainService.srchOne(traid);
            if (tra == null){
                res.setSuccess(ResponseBean.FAIL);
                res.setErrormsg("该培训主体信息丢失");
                return res;
            }

            WhgSysUser sysUser = (WhgSysUser) session.getAttribute("user");
            this.whgTrainCourseService.t_add(tra,course, sysUser);
        }catch (Exception e){
            res.setSuccess(ResponseBean.FAIL);
