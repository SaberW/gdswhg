...
public class WhgActivitySeatService {
	 ...
	 * @throws Exception
	 */
	public int t_edit(List<Map<String, String>> seatList, WhgSysUser user,String actId)throws Exception{
		//设置初始值
		Date now = new Date();
		int num = 0;
		for (int i = 0; i < seatList.size(); i++) {
			...
			WhgActSeat record = new WhgActSeat();
			record.setSeatstatus(seatstatus);
			Example example = new Example(WhgActSeat.class);
			example.createCriteria().andEqualTo("activityid", actId)
					.andEqualTo("seatrow", seatrow).andEqualTo("seatcolumn", seatcolumn);
			this.whgActSeatMapper.updateByExampleSelective(record, example);
			if(seatstatus == 1){
				num++;
			}
		}
     ...
     * @throws Exception
     */
    public int t_add(List<Map<String, String>> seatList, WhgSysUser user,String actId)throws Exception{
        //设置初始值
        Date now = new Date();
        int num = 0;//计算有效座位数
        for (int i = 0; i < seatList.size(); i++) {
