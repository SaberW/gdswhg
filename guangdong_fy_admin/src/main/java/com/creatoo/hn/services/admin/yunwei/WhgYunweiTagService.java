...
public class WhgYunweiTagService extends BaseService{
     ...
     */
    @Cacheable
    public PageInfo<WhgYwiTag> t_srchList4p(int page,int rows, WhgYwiTag whgYwiTag) throws Exception {
        //开始分页
        PageHelper.startPage(page, rows);
        Example example = new Example(WhgYwiTag.class);
        Example.Criteria c = example.createCriteria();

        if ( whgYwiTag.getType() != null && !"".equals(whgYwiTag.getType())) {
            c.andEqualTo("type", whgYwiTag.getType());
        }
        if ( whgYwiTag.getCultid() != null && !"".equals(whgYwiTag.getCultid())) {
