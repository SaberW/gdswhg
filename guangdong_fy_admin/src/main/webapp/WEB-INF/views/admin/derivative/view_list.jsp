...
<body>
<!-- 表格 -->
<table id="whgdg" title="非遗衍生品管理" class="easyui-datagrid" style="display: none"
       data-options="fit:true, striped:true, rownumbers:true, fitColumns:true, singleSelect:false, checkOnSelect:true, selectOnCheck:true, pagination:true, toolbar:'#whgdg-tb', url:'${basePath}/admin/derivative/derivativeList'">
    <thead>
    <tr>
        <th data-options="field:'name', width:60">名称</th>
        <th data-options="field:'etype', width:60 ,formatter:WhgComm.FMTVenueType">非遗类型</th>
...
<script type="text/javascript">
     ...
     */
    function add() {
        WhgComm.editDialog('${basePath}/admin/derivative/view/add');
    }
    ...
    function edit(idx) {
        ...
        WhgComm.editDialog('${basePath}/admin/derivative/view/edit?id='+curRow.id);
    }
    ...
    function lookAll(idx) {
        var curRow = $('#whgdg').datagrid('getRows')[idx];
        WhgComm.editDialog('${basePath}/admin/derivative/view/edit?targetShow=1&id='+curRow.id);
    }
    ...
    function __updrecommend(ids, toState) {
        ...
        $.post('${basePath}/admin/derivative/updCommend', params, function(data){
    ...
    function __updStateSend(ids, formstates, tostate){
        $.ajax({
            url: getFullUrl("/admin/derivative/updstate"),
    ...
    function del(idx) {
        ...
        $.messager.confirm('提示', '您确定要删除此记录吗？', function(r){
            if (r){
                $.ajax({
                    type: "POST",
                    url: getFullUrl('/admin/derivative/del'),
                    data: {id : curRow.id},
