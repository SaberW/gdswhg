...
<form id="whgff" method="post" class="whgff">
    ...
    <div class="whgff-row">
        ...
        <div class="whgff-row-input">
            <input class="easyui-combobox" name="teachername" value="${whgTra.teachername}" style="width:500px; height:32px" data-options="multiple:true,editable:true,valueField:'name',textField:'name',url:'${basePath}/admin/train/tea/srchList'
            ,onChange: function (val, oldval) {
                if (val.length>1 && val[0]==''){
                    val.shift();
                    $(this).combobox('setValues', val);
    ...
    <div class="whgff-row">
        <div class="whgff-row-label"><label style="color: red">*</label>直播分类：</div>
        <div class="whgff-row-input">
        ...
        <c:forEach items="${course}" var="item" varStatus="s" end="0">
            <div class="whgff-row single_edit" >
                ...
                <div class="whgff-row-input">
                    ...
                    <input class="easyui-datetimebox sin_endtime_edit" id="sin_endtime_edit" name="sin_endtime" value='<fmt:formatDate value="${item.endtime}" pattern="yyyy-MM-dd HH:mm:ss"/>' style="width:200px; height:32px" data-options="validType:['sinedit_Time[\'sin_starttime_edit\']', 'oneDay[\'sin_starttime_edit\']']"/>
                </div>
            </div>
        </c:forEach>
    </c:if>
    ...
    <div class="whgff-row">
        <div class="whgff-row-label"><label style="color: red">*</label>温馨提示：</div>
        <div class="whgff-row-input">
            <input class="easyui-textbox" name="notice" value="${whgTra.notice}" style="width:600px; height:100px" data-options="multiline:true, validType:['length[1,1000]'],prompt:'请填写温馨提示'">
        </div>
    </div>
...
<script>
    ...
    function myChangeProvince(newVal, oldVal) {
        ...
        if(__init_city){
            window.setTimeout(function () {$('#__CITY_ELE').combobox('setValue', '${whgTra.city}');}, 500);
            __init_city = false;
        }
    }
    ...
    function myChangeCity(newVal, oldVal) {
        ...
        if(__init_area){
            window.setTimeout(function () {$('#__AREA_ELE').combobox('setValue', '${whgTra.area}')}, 500);
            __init_area = false;
        }
    }  //省市区------END
    ...
    $(function () {
        WhgComm.initPMS({
            basePath:'${basePath}',
            cultEid:'cultid', cultValue:'${whgTra.cultid}',
