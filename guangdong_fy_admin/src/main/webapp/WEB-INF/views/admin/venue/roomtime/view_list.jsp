        ...
        var addDialog =$('<div></div>').dialog({
            ...
            onOpen: function () {
                ...
                $.getJSON('${basePath}/admin/venue/roomtime/ajaxAddStartDay?t='+(new Date()).getTime(), {roomid: roomid}, function(data){
                    ...
                    $(that).find(".easyui-datebox:eq(0)").attr('validType', "startDate['"+WhgComm.FMTDate(sday)+"']");
                    $.parser.parse($(that));
                    $(that).find(".easyui-datebox:eq(0)").datebox('setValue', WhgComm.FMTDate(sday));
        ...
        var optDialog =$('<div></div>').dialog({
            ...
            onOpen: function () {
                $(this).find(".dialog-content-add").append('\
                    ...
                    <br/>是否收费：<select class="easyui-combobox" panelHeight="auto" data-options="width:150, required:true, editable:false">\
