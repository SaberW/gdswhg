...
<body>
<!-- 表格 -->
<table id="whgdg" title="公文档案列表" class="easyui-datagrid" style="display: none"
       data-options="fit:true, striped:true, rownumbers:true, fitColumns:true, singleSelect:false, checkOnSelect:true, selectOnCheck:true, pagination:true, toolbar:'#whgdg-tb',queryParams:{cultid: WhgComm.getMgrCults()[0].id}, url:'${basePath}/admin/fyi/archive/srchList4p'">
    <thead>
    <tr>
        <th data-options="field:'title',width:80">标题</th>
        <th data-options="field:'format',width:80,formatter:FMTGS">格式</th>
...
<script>
     ...
     */
    function add(){
        WhgComm.editDialog('${basePath}/admin/fyi/archive/view/add');
    }
    ...
    function edit(idx){
        ...
        WhgComm.editDialog('${basePath}/admin/fyi/archive/view/add?id='+row.id);
    }
    ...
    function view(idx){
        var row = $("#whgdg").datagrid("getRows")[idx];
        WhgComm.editDialog('${basePath}/admin/fyi/archive/view/add?show=1&id='+row.id);
    }
    ...
    function del(idx){
        ...
        $.messager.confirm("确认信息", confireStr, function(r){
            if (r){
                $.messager.progress();
                $.post('${basePath}/admin/fyi/archive/del', {id: row.id}, function(data){
