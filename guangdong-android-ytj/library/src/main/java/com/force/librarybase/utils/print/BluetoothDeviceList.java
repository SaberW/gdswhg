...
/*        Set<BluetoothDevice> pairedDevices = mBtAdapter.getBondedDevices();//set集合中不允许重复的出现
        if (pairedDevices.size() > 0) {
            for (BluetoothDevice device : pairedDevices) {
                mPairedDevicesArrayAdapter.add(device.getName() + "( 已绑定 )"
                        + "\n" + device.getAddress());
            }
        }*/
        doDiscovery();
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (BluetoothDevice.ACTION_FOUND.equals(action)) {
                ...
                itemName = itemName  + "( "
                        + (
                                device.getBondState() == BluetoothDevice.BOND_BONDED ? "已绑定"//结合
                                        : "未绑定") + " )"
                        + "\n" + device.getAddress();
                ...
                scanButton.setText("扫描新设备"); // setTitle("请选择要连接的设备");
                if (mPairedDevicesArrayAdapter.getCount() == 0) {
                    String noDevices = "noFoundDevice";
