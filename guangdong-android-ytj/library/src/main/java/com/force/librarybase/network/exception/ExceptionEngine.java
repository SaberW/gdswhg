    ...
    public static ApiException handleException(Throwable e){
        ...
        if (e instanceof HttpException){             //HTTP错误
            ...
            switch(httpException.code()){
                case UNAUTHORIZED:
                    ex.setMsg("当前请求需要用户验证");
                    break;
                case FORBIDDEN:
                    ex.setMsg( "服务器已经理解请求，但是拒绝执行它");
                    break;
                case NOT_FOUND:
                    ex.setMsg( "服务器异常，请稍后再试");;
                    break;
                case REQUEST_TIMEOUT:
                    ex.setMsg("请求超时");
                    break;
                case GATEWAY_TIMEOUT:
                    ex.setMsg("作为网关或者代理工作的服务器尝试执行请求时，未能及时从上游服务器（URI标识出的服务器，例如HTTP、FTP、LDAP）或者辅助服务器（例如DNS）收到响应");
                    break;
                case INTERNAL_SERVER_ERROR:
                    ex.setMsg( "服务器遇到了一个未曾预料的状况，导致了它无法完成对请求的处理");
                    break;
                case BAD_GATEWAY:
                    ex.setMsg("作为网关或者代理工作的服务器尝试执行请求时，从上游服务器接收到无效的响应");
                    break;
                case SERVICE_UNAVAILABLE:
                    ex.setMsg("由于临时的服务器维护或者过载，服务器当前无法处理请求");
                    break;
                default:
                    ex.setMsg("网络错误");  //其它均视为网络错误
                    break;
            }
            return ex;
        } else if (e instanceof ServerException){    //服务器返回的错误
            ServerException resultException = (ServerException) e;
            ex = new ApiException(resultException,Integer.parseInt(resultException.getCode()) );
