 ...
 * @author way
 */
public class CrashCaptureUtils implements UncaughtExceptionHandler {
    private static final String TAG = "CrashCaptureUtils";
    private UncaughtExceptionHandler mDefaultHandler;// 系统默认的UncaughtException处理类
    private static CrashCaptureUtils INSTANCE = new CrashCaptureUtils();// CrashHandler实例
     ...
     */
    public static CrashCaptureUtils getInstance() {
        return INSTANCE;
    }
    ...
    public boolean handleException(Throwable ex) {
        ...
        new Thread() {
            public void run() {
                Looper.prepare();
                ToastUtils.show(mContext, "主子！很抱歉,累了,休息一会！");
                Looper.loop();
            }
        }.start();
        // 收集设备参数信息
//        collectDeviceInfo(mContext);
    ...
    private String getFilePath() {
        ...
        if (isSDCardExist && isRootDirExist) {
            ...
            file_dir = CAApplication.getInstance().getFilesDir().getAbsolutePath() + "/crashlog/";
        }
        return file_dir;
    }
