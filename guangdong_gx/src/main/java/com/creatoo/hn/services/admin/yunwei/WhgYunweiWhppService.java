...
//        c.andEqualTo("delstate", 0);
    ...
    public PageInfo<WhgYwiWhpp> findBrand(Map map)throws Exception{
        ...
        if(map != null &&map.get("cultid") != null){
            c.andIn("cultid", (List)map.get("cultid"));
        }
        if(map != null &&map.get("province") != null){
            c.andEqualTo("province", map.get("province").toString());
        }
        if(map != null &&map.get("city") != null){
            c.andEqualTo("city", map.get("city").toString());
        }
        if(map != null &&map.get("area") != null){
            c.andEqualTo("area", map.get("area").toString());
        }

        if (map !=null && map.get("pptype") != null){
            c.andEqualTo("pptype", map.get("pptype"));
        }
        if (map !=null && map.get("keywords")!=null && !map.get("keywords").toString().isEmpty()) {
            String keywords = map.get("keywords").toString();
            c.andCondition(" (name like '%"+keywords+"%' or shortname like '%"+keywords+"%') ");
        }
    ...
    public void t_add(HttpServletRequest request, WhgYwiWhpp wyl) throws Exception {
        ...
        whgYwiWhpp.setProvince(wyl.getProvince());
        whgYwiWhpp.setCity(wyl.getCity());
        whgYwiWhpp.setArea(wyl.getArea());
        ...
        whgYwiWhpp.setPptype(wyl.getPptype());
