...
public class WhgYunweiKeyService extends BaseService{
     ...
     */
    @Cacheable
    public PageInfo<WhgYwiKey> t_srchList4p(int page,int rows, WhgYwiKey whgYwiKey, String userId) throws Exception {
        //开始分页
        ...
        if(whgYwiKey == null){
            whgYwiKey = new WhgYwiKey();
        }
        if(StringUtils.isNotEmpty(whgYwiKey.getCultid())) {
        ...
        if (StringUtils.isNotEmpty(whgYwiKey.getType())) {
            c.andEqualTo("type", whgYwiKey.getType());
        }
        if(StringUtils.isNotEmpty(whgYwiKey.getName())){
