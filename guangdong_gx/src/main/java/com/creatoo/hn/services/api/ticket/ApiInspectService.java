...
@Service
public class ApiInspectService extends BaseService{
    ...
    public Map<String, Object> loginCheckSysUser(String username, String password) throws Exception{
        ...
        WhgSysDept dept = null;//this.whgSysDeptMapper.selectByPrimaryKey(user.getDeptid());
    ...
    public Map<String, Object> orderActivityCode(String orderValidateCode) throws Exception{
        ...
        //订单
        if (actOrder.getTicketstatus() != null && actOrder.getTicketstatus() == 3) {
            rest.put("status", "RESULT_ACTIVITY_ERROR_CODE_10004"); //订单取消
            return rest;
        }

        //活动和场次
        WhgActivity act = this.whgActActivityMapper.selectByPrimaryKey(actOrder.getActivityid());
        WhgActivityTime time = this.whgActTimeMapper.selectByPrimaryKey(actOrder.getEventid());
        ...
        //订单对应的票
        WhgActivityTicket ticket = new WhgActivityTicket();
        ticket.setOrderid( actOrder.getId() );
        List<WhgActivityTicket> list= this.whgActTicketMapper.select(ticket);
    ...
    public Map<String, Object> activityCode(String userId, String orderValidateCode, String userAccount,
        ...
        c2.andEqualTo("orderid",actOrder.getId());
        c2.andEqualTo("ticketstatus", 0);
        int count = this.whgActTicketMapper.selectByExample(example2).size();
        if(count==0){
            actOrder.setTicketstatus(4);//订单信息中加入 已验票 状态
            this.whgActOrderMapper.updateByPrimaryKey(actOrder);
        }
        return rest;
    }
