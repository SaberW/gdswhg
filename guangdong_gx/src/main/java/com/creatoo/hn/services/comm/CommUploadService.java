 ...
 */
@Service
public class CommUploadService extends BaseService {
     ...
     * @throws Exception
     */
    public UploadFileBean uploadFile(String path,MultipartFile multiFile, String uploadFileType, boolean needCut, String cutWidth, String cutHeight, String waterFilePath)throws Exception{
...
//            String file = suffix.toLowerCase();
            ...
            //使用本地上传还是其它方式上传
            String useType = env.getProperty("upload.type");//commPropertiesService.getUploadType();
            if("local".equals(useType)){
                //保存到本地
                String rootPath = env.getProperty("upload.local.addr");//commPropertiesService.getUploadLocalAddr();
                    ...
                    if(!needCut){
                        ...
                        BufferedImage waterbi =null;
                            ...
                            try {
                                ...
                                waterPic = new File(waterFilePath);
                                waterbi = ImageIO.read(waterPic);
                            }
                ...
                //oss配置
                String endpoint = env.getProperty("upload.oss.endpoint");//commPropertiesService.getUploadOssEndpoint();//AliyunOssUtil.getEndpoint();// endpoint以杭州为例，其它region请按实际情况填写
                String accessKeyId = env.getProperty("upload.oss.access.key.id");//commPropertiesService.getUploadOssAccessKeyId();//AliyunOssUtil.getAccessKeyId();// accessKey请登录https://ak-console.aliyun.com/#/查看
                String accessKeySecret = env.getProperty("upload.oss.access.key.secret");//commPropertiesService.getUploadOssAccessKeySecret();//AliyunOssUtil.getAccessKeySecret();
                String bucketName = env.getProperty("upload.oss.bucket.name");//commPropertiesService.getUploadOssBucketName();//AliyunOssUtil.getBucketName();
     ...
     * @throws Exception
     */
    public UploadFileBean uploadFile(InputStream inputStream, String  oldFileName)throws Exception{
        if(null == inputStream){
            return null;
        }
        UploadFileBean uploadFileBean = new UploadFileBean();
        ...
        try{
            ...
            /**
             ...
             uploadFileBean.setWidth(bimg.getWidth());
             uploadFileBean.setHeight(bimg.getHeight());
             if( inputStream.available() > (1048576*2) ){
             uploadFileBean.setSuccess(UploadFileBean.FAIL);
             uploadFileBean.setErrormsg("图片大小必须在2MB以内");
             return uploadFileBean;
             }
            ...
            // 获取文件后缀
            String suffix = "";
            System.out.println("1------------" + oldFileName);
            int idx = oldFileName.lastIndexOf(".");
            ...
            if("local".equals(useType)){
                //保存到本地
                String rootPath = env.getProperty("upload.local.addr");//commPropertiesService.getUploadLocalAddr();
                File dir = new File(rootPath, "img"+File.separator+yyyy+File.separator+yyyyMM+File.separator+yyyyMMdd);
                if(!dir.exists()){
                    dir.mkdirs();
                }
                File uploadFile = new File(dir, newFileName);
                ...
                String newFileName_750_500 = getFileName(newFileName, "750", "500");
                File newFile_750_500 = new File(dir, newFileName_750_500);
                Thumbnails.of(uploadFile).size(750, 500).keepAspectRatio(false).outputQuality(1.0).toFile(newFile_750_500);

                //740_555
                String newFileName_740_555 = getFileName(newFileName, "740", "555");
                File newFile_740_555 = new File(dir, newFileName_740_555);
                Thumbnails.of(uploadFile).size(740, 550).keepAspectRatio(false).outputQuality(1.0).toFile(newFile_740_555);

                //300_200
                String newFileName_300_200 = getFileName(newFileName, "300", "200");
                File newFile_300_200 = new File(dir, newFileName_300_200);//.watermark(Positions.BOTTOM_RIGHT, ImageIO.read(new File("watermark.png")), 0.5f)
                Thumbnails.of(uploadFile).size(300, 200).keepAspectRatio(false).outputQuality(1.0).toFile(newFile_300_200);

                //multiFile.transferTo(uploadFile);

                //最终上传文件的地址
                uploadFileBean.setUrl("/img/"+yyyy+"/"+yyyyMM+"/"+yyyyMMdd+"/"+newFileName);
            }else{ //上传到oss
                //oss配置
                String endpoint = env.getProperty("upload.oss.endpoint");//commPropertiesService.getUploadOssEndpoint();//AliyunOssUtil.getEndpoint();// endpoint以杭州为例，其它region请按实际情况填写
                String accessKeyId = env.getProperty("upload.oss.access.key.id");//commPropertiesService.getUploadOssAccessKeyId();//AliyunOssUtil.getAccessKeyId();// accessKey请登录https://ak-console.aliyun.com/#/查看
     ...
     * @return
     */
    public UploadFileBean uploadImgCut(String path,String imgurl, String x, String y, String x2, String y2, String w, String h, String waterFilePath)throws Exception {
