...
public class WhgInfoController extends BaseController {
    /**
     * 公用资讯serivice
     */
    @Autowired
    ...
    public ModelAndView view(HttpServletRequest request, @PathVariable("type") String type, String entityid, String entity){
        ...
        try {
            if("list".equalsIgnoreCase(type)){
                view.setViewName("admin/information/info/view_list");
            }
            if("add".equalsIgnoreCase(type)){
                view.setViewName("admin/information/info/view_add");
            }
...
//            }
            if("edit".equalsIgnoreCase(type)){
                if(id != null){
                    ...
                    if(clnftype.equals("2016111900000021") || clnftype.equals("2016111900000018")){
                        view.addObject("type", 1);
                    }
                    if(clnftype.equals("2016111900000020") || clnftype.equals("2016111900000012")){
                        view.addObject("type", 2);
                    }
                    view.setViewName("admin/information/info/view_edit");
                }
            }
        }catch (Exception e) {
    ...
    public ResponseBean srchcolinfoList4p(HttpServletRequest request){
        ...
        try {
            ...
            PageInfo pageInfo = this.whgInfoService.t_srchcolinfoList4p(paramMap,whgSysUser);
            resb.setRows( (List)pageInfo.getList() );
            resb.setTotal(pageInfo.getTotal());
        } catch (Exception e) {
    ...
    @RequestMapping("/add")
    @ResponseBody
    public ResponseBean add(WhgInfColinfo info, HttpServletRequest request, HttpSession session, String entityid, String entity, String type,Integer ispub) {
        ResponseBean res = new ResponseBean();
        WhgSysUser user = (WhgSysUser) request.getSession().getAttribute(Constant.SESSION_ADMIN_KEY);
        getClnftype(info, entity, type);
        try {
            this.whgInfoService.t_add(info,user,request,entityid,ispub);
        } catch (Exception e) {
     ...
     */
    @RequestMapping("/edit")
    public ResponseBean edit(WhgInfColinfo info, HttpServletRequest request, HttpSession session, String entityid, String entity, String type,Integer ispub){
        ResponseBean res = new ResponseBean();
        ...
        getClnftype(info, entity, type);
        try {
            this.whgInfoService.t_edit(info, sysUser, request, entityid,ispub);
        } catch (Exception e) {
            res.setSuccess(ResponseBean.FAIL);
    ...
    public void getClnftype(WhgInfColinfo info,String entity, String type){
        if(entity.equals("1")){
            if(type.equals("1")){
        ...
        if(entity.equals("4")){
            info.setClnftype("2017101276534290");
        }
    }
