...
public class VisitAction {
    ...
    /**
     * 保存访问记录
     * <script src="http://localhost:8080/static/admin/js/pc.js?type=1" id="whgtongji"></script>
     ...
     * @return
     */
    @RequestMapping(value = "/getcount4url", method = {RequestMethod.GET, RequestMethod.POST})
    public ResponseBean getVisitCount4url(String urlfag){
        ResponseBean rb = new ResponseBean();
        try {
            long count = this.visitService.getVisitCount4Url(urlfag);
            rb.setData(count);
        } catch (Exception e) {
            rb.setData(0);
            rb.setSuccess(ResponseBean.FAIL);
