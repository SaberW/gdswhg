...
import com.creatoo.hn.services.admin.train.WhgTeaInMajorService;
 ...
 */
@Controller
@RequestMapping("admin/major/tea")
public class WhgTeaInMajorController extends BaseController{

    @Autowired
    private WhgTeaInMajorService whgTeaInMajorService;
    ...
    public ModelAndView view(HttpServletRequest request, @PathVariable("type") String type){
        ...
        view.setViewName("admin/train/major/tea/view_"+type);
        return view;
    }
    ...
    public ResponseBean srchList4p(int page, int rows, WebRequest request){
        ...
        String mid = request.getParameter("mid");
        try {
            PageInfo pageInfo = this.whgTeaInMajorService.t_srchlist4p(page,rows,mid);
    ...
    public Object saveTraintpl(HttpServletRequest req, String id){
        ...
        try {
            this.whgTeaInMajorService.t_del(id);
    ...
    public ResponseBean add(String entid,String mid) {
        ResponseBean res = new ResponseBean();
        try {
            this.whgTeaInMajorService.t_add(entid,mid);
        } catch (Exception e) {
            res.setSuccess(ResponseBean.FAIL);
