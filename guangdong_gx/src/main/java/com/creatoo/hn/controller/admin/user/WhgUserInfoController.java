...
public class WhgUserInfoController extends BaseController {
        ...
        try {
            ...
            try{
                ...
                if(myuser.getIsrealname() != null && myuser.getPhone() != null){
                    ...
                    String tempCode = (user.getIsrealname() == 1)? "REL_CHECK_PASS" : "REL_CHECK_FAIL";
                    smsService.t_sendSMS(myuser.getPhone(), tempCode, data, user.getId());

                    StringBuilder sb = new StringBuilder();
                    for(EnumProject ep : EnumProject.values()){
                        if (sb.length() > 0){
                            sb.append(",");
                        }
                        sb.append(ep.getValue());
                    }
                    insMessageService.t_sendZNX(myuser.getId(),null, tempCode, data, myuser.getId(), sb.toString());
                }
            }catch (Exception e){
                log.error(e.getMessage(),e);
            }
     ...
     * @return
     */
    @RequestMapping(value = "/setuserphone")
    @ResponseBody
    @WhgOPT(optType = EnumOptType.MEMBER, optDesc = {"更改手机"})
    public ResponseBean setUserPhone(String id, String phone){
        ResponseBean res = new ResponseBean();

        try {
            res = this.userService.setUserPhone(id, phone);
        } catch (Exception e) {
            log.error(e.getMessage(), e);
            res.setSuccess(ResponseBean.FAIL);
