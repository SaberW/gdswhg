...
initParams={
        @WebInitParam(name="allow",value="192.168.16.110,127.0.0.1,106.15.57.130"),// IP白名单 (没有配置或者为空，则允许所有访问)
...
public class PostObjectPolicy extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            String libid = request.getParameter("libid");//资源库标识
            String uid = request.getParameter("uid");//管理员标识
            String nocallback = request.getParameter("nocallback");//不需要回调
            ...
            //回调配置
            if(!"1".equals(nocallback)) {
                com.alibaba.fastjson.JSONObject json = new com.alibaba.fastjson.JSONObject();
                json.put("callbackUrl", getCallbackUrl(request) + "?libid=" + libid + "&uid=" + uid);//回调函数
                json.put("callbackHost", getCallbackHost(request));
                json.put("callbackBody", "filename=${object}&size=${size}&mimeType=${mimeType}&height=${imageInfo.height}&width=${imageInfo.width}");
                json.put("callbackBodyType", "application/x-www-form-urlencoded");
                String callback = json.toJSONString();
                byte[] encodeStr = Base64.encodeBase64(callback.getBytes("utf-8"));
                String callbackbody = new String(encodeStr);
    ...
    private String getCallbackUrl(HttpServletRequest request){
        ...
        System.out.println(basePath+"/admin/mass/libres/notify");
        return basePath+"/admin/mass/libres/notify";
    }
     ...
     * @return 服务器域名
     */
    private  String getCallbackHost(HttpServletRequest request){
        return request.getServerName();
    }

    public static void main(String[] args)throws Exception {
        com.alibaba.fastjson.JSONObject json = new com.alibaba.fastjson.JSONObject();
        json.put("callbackUrl", "http://172464m4q6.iok.la/admin/mass/libres/notify");//回调函数
        json.put("callbackHost", "172464m4q6.iok.la");
        json.put("callbackBody", "filename=${object}&size=${size}&mimeType=${mimeType}");
        json.put("callbackBodyType", "application/x-www-form-urlencoded");
        String callback = json.toJSONString();
        byte[] encodeStr = Base64.encodeBase64(callback.getBytes("utf-8"));
        String encode = new String(encodeStr);
