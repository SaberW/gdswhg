...
WhgIDUtil = (function(){
    var __id = 100;
    function _getId(){ return __id++; }
    return {
        getId: function(){ return _getId(); }
    }
})();
...
$.extend($.fn.validatebox.defaults.rules, {
        ...
        validator: function(value, param){
            if(value != ''){
                ...
                if(rex.test(value)){
                    ...
                    $.fn.validatebox.defaults.rules.isTel.message ="请输入正确的固定电话, 如：010-8888888.";
                    return false;
                }
        ...
        validator: function(value, param){
            if(value != ''){
                ...
                if(rex.test(value) || rex2.test(value)) {
                    ...
                    $.fn.validatebox.defaults.rules.isPhoneTel.message ="请输入正确的联系方式, 如：13688888888或010-8888888";
                    return false;
                }
    ...
    isIdcard: {
        validator: function(value, param){
            if(value != ''){
                var rex = /^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}([0-9]|X|x)$/;
                if(rex.test(value)){
                    return true;
                }else{
                    $.fn.validatebox.defaults.rules.isIdcard.message ="请输入正确的身份证号码";
                    return false;
                }
    ...
    isVideo: {
    	validator: function(value, param){
        	...
        	if(fileObj && fileObj.files && fileObj.files.length > 0){
	            ...
	            if($.inArray(file_typename, allowArr) < 0){
	            	//错误消息
	            	$.fn.validatebox.defaults.rules.isVideo.message ="请选择正确的文件格式(mp4).";
	            	return false;
	            }
	            ...
	            if(fileSize > size){
	            	if(size >= 1048576){
	            		var tsize = (size/1024).toFixed(2);
	            		$.fn.validatebox.defaults.rules.isVideo.message ="请选择正确的文件格式(mp4), 大小限制在"+tsize+"M以内.";
	            	}else{
	            		$.fn.validatebox.defaults.rules.isVideo.message ="请选择正确的文件格式(mp4), 大小限制在"+size+"KB以内.";
	            	}
	            	return false;
	            }
        	}
 ...
 * 根据区域值获取省市区
 * @param area
 * @returns {{province: string, city: string, area: string}}
 */
function getProvinceCityArea(area){
	var areaInfo = {"province":"", "city":"", "area":"", "level":""};
    $.ajax({
    	type: 'POST',
        url: getFullUrl('/admin/yunwei/area/areaInfo'),
        data: {area:area},
        cache: false,
        async: false,
        success: function (data) {
 ...
 * 区域树
 */
WhgCommAreaTree = (function(){
    /**
     * 构造easyui-tree或者easyui-combotree
     * @param options.eleId 构造easyui-truee的ul元素的id属性值
     * @param options.loadCult 是否加载文化馆
     * @param options.selectFirstNode 默认选中第一个节点
     * @param options.selectFirstCult 默认选中第一个文化馆
     * @param options.onSelect 选中节点后触发的函数，如: function selectFun(node, areaInfo){}
     * @param options.onBeforeSelect 选中节点后触发的函数，如: function selectFun(node, areaInfo){}
     * @param options.canSelectArea 能否选择省市区
     * @param options.canSelectCult 能否选择文化馆
     */
	function _init(options, treeType) {
        //默认参数
        ...
        //局部属性
        var firstSelect = false;//默认是否选中了第一个节点
        var firstLevel =  -1; //默认是否获得了第一层节点的level

        //Tree 参数
        var tree_options = {
            url: this_options.loadCult ? basePath+'/admin/yunwei/area/srchList4tree4cult' : basePath + '/admin/yunwei/area/srchList4tree',
            animate: true,
            lines: true,
            onBeforeExpand: function(node){
                var t = $(this);
                t.tree('options').queryParams = {level:node.level, area:node.text};
            },
            onLoadSuccess: function (node, data) {
                ...
                if(data){
                    $(data).each(function(index, d){
                        ...
                        if(firstLevel == -1){//第一层节点的级别
                            firstLevel = this.level;
                        }
                        if(this_options.selectFirstNode){//默认选中第一个节点
                            if(!firstSelect){
                                t.tree('select', n.target);
                                firstSelect = true;
                            }
                        }
                        if(this_options.selectFirstCult){//默认选中第一个文化馆
                            if(!firstSelect && typeof(this.iscult) != 'undefined'){
                                t.tree('select', n.target);
                                firstSelect = true;
                            }
                        }
                        if(typeof(d.iscult) != 'undefined' && d.iscult == '1'){
                            $(n.target).addClass('isCultNode');
                        }else{
                            $(n.target).addClass('isAreaNode');
                        }
                        if(this.state == 'closed' && this.level <= firstLevel){
                            t.tree('expandAll');
                        }
                    });
                }
            ...
            onBeforeSelect: function (node) {
                var canSelect = false;
                if(node && typeof(node.iscult) != 'undefined' && node.iscult == '1'){//文化馆节点
                    canSelect = this_options.canSelectCult;
                }else{//区域节点
                    canSelect = this_options.canSelectArea;
                }
                if(canSelect && $.isFunction(this_options.onBeforeSelect)){
                    canSelect = this_options.onBeforeSelect(node);
                }
                return canSelect;
            },
            onSelect: function(node){
                if($.isFunction(this_options.onSelect)){
                    if(node && typeof(node.iscult) != 'undefined' && node.iscult == '1') {//文化馆节点
                        this_options.onSelect(node, firstLevel);
                    }else{
                    	this_options.onSelect(node, firstLevel, getProvinceCityArea(node.text));
					}
                }
            }
        };

        if('combotree' == treeType){
            $('#'+this_options.eleId).combotree(tree_options);
		}else {
            $('#'+this_options.eleId).tree(tree_options);
		}
    }
     ...
     * @param options.canSelectCult 能否选择文化馆
     */
    function _initAreaCombotree(options, treeType) {
		_init(options, 'combotree');
    }
     ...
     * @param options.canSelectCult 能否选择文化馆
     */
    function _initAreaTree(options){
        _init(options, 'tree');
	}
	...
	return {
         ...
         * @param options.selectFirstNode 默认选中第一个节点
         * @param options.selectFirstCult 默认选中第一个文化馆
         * @param options.onSelect 选中节点后触发的函数，如: function selectFun(node, firstLevel, areaInfo){}
         * @param options.onBeforeSelect 选中节点后触发的函数，如: function selectFun(node, areaInfo){}
         * @param options.canSelectArea 能否选择省市区
         * @param options.canSelectCult 能否选择文化馆
         */
        initAreaTree: function(options){
            _initAreaTree(options);
		}

        /**
         * 构造省市区的树easyui-combotree
         * @param options.eleId 构造easyui-truee的ul元素的id属性值
         * @param options.loadCult 是否加载文化馆
         * @param options.selectFirstNode 默认选中第一个节点
         * @param options.selectFirstCult 默认选中第一个文化馆
         ...
         * @param options.canSelectCult 能否选择文化馆
         */
		,initAreaCombotree: function (options) {
            _initAreaCombotree(options);
        }

        /**
		 * 根据eleId获取easyui-tree对象
         * @param eleId
         * @returns {*|jQuery|HTMLElement}
         */
		,getTree: function (eleId) {
			return $('#'+eleId);
        }
	};
})();