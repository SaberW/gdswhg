...
function send_request()
{
    ...
    if (xmlhttp!=null)
    {
        serverUrl = basePath+'/signature?dir='+encodeURIComponent(_userDir)+'&nocallback=1&t='+new Date().getTime();
...
var uploader = new plupload.Uploader({
    ...
    filters: {
        mime_types : [ //只允许上传图片和zip,rar文件
	        ...
	        { title : 'Video', extensions : "mp3,mp4"}
        ],
	...
	init: {
		...
		Error: function(up, err) {
            if (err.code == -600) {
                document.getElementById('console').appendChild(document.createTextNode("选择的视频文件过大，最多支持500M。"));
            }else if (err.code == -601) {
                document.getElementById('console').appendChild(document.createTextNode("请选择有效的视频格式文件,支持的视频格式有mp3,mp4。"));
            }else if (err.code == -602) {
                document.getElementById('console').appendChild(document.createTextNode("这个文件已经上传过一遍了。"));
            }else{
                ...
                //document.getElementById('console').appendChild(document.createTextNode("错误消息:" + err.response));
            }
		}
	}
});
