...
WhgUploadImg = (function () {
    ...
    function __obj_Construct(options){
        ...
        this._options = $.extend({}, {
            ...
            needCut: true,                                          //是否裁剪图片
            isSingleSy: true,                                          //是否调用自己的水印  默认都加上
            ...
            allowUpload: true,                                      //允许上传，如果不允许上传，只设置预览图片
        }, options);
    ...
    __obj_Construct.prototype._goCutImg = function (imgUrl) {
        ...
        var cut_img_url = this._options.basePath+"/comm/cutImg?isSingleSy="+this._options.isSingleSy+"&w="+this._options.cutWidth+"&h="+this._options.cutHeight+"&imgurl="+encodeURIComponent(imgUrl)+"&ww="+_ww+"&wh="+_wh;
        if( $('#'+id_dialog).size() > 0 ){
        ...
        $('#'+id_dialog+'_ok').on("click", function () {
            ...
            $.ajax({
                ...
                data: {isSingleSy:thisObj._options.isSingleSy,imgurl:thisObj._uploadSuccFileURL,x:cutObj.x, y:cutObj.y, x2:cutObj.x2, y2:cutObj.y2, w:thisObj._options.cutWidth, h:thisObj._options.cutHeight},
    ...
    __obj_Construct.prototype._beforeUpload = function (file) {
        this._uploader.setOption({
            'multipart_params': {
                ...
                isSingleSy: this._options.isSingleSy,
