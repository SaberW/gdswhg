...
function addzy(index){
	...
	var id = row.clnfid;
	WhgComm.editDialog('${basePath}/admin/information/uploads?rsid=${resourceid}&refid='+id);
}
/**
 * 添加资源
 * @param index
...
function addzl(idx) {
	...
	WhgComm.editDialog('${basePath}/admin/resource/view/list?reftype=11&refid=' + row.clnfid+'&cultid='+cultid);
}
...
//				{field:'img',title:'图片',width:100,formatter:showImg},
				{field:'clnfstata',title:'状态',width:100,sortable:true,formatter: WhgComm.FMTBizState},
//				{field:'clnfidx',title:'排序',width:50},
	...
	function initGridPanel(node){
		...
		$(_grid).datagrid({
			url: getFullUrl('/admin/information/seleinfo'),
	...
	function addinfo(){
		...
		var _frm = editWinform.getWinBody().find("form").form({
			url : getFullUrl('/admin/information/addmusinfo'),
	...
	function delinfo(idx){
		...
		$.messager.confirm('确认对话框', '您确认要删除吗？', function(r){
			if (r){
				$.ajax({
					type: "POST",
					url: getFullUrl('/admin/information/delinfo'),
	...
	function _oneSetState(id, state){
		$.ajax({
    		type: "POST",
     		url : getFullUrl("./admin/information/checkinfo"),
...
//                     $.messager.alert("提示","操作成功");
	...
	function _batchSend(data){
		$.ajax({
			type: "POST",
			url: getFullUrl('./admin/information/checkeinfos'),
			data: data,
			success: function(data){
			   if(data.success == '0'){
					$(_grid).datagrid('reload');
...
function goindex(index){
	...
	var frm = $('#ff_').form({
		url : getFullUrl("/admin/information/goinfo"),
		onSubmit : function(param) {
			param.clnfid = row.clnfid;
	...
	<div style="display:none" data-options="region:'center',title:'',iconCls:'icon-ok'">
		...
		<div id="infTB" style="height:auto" style="display:none">
			...
			<div>
				...
				<%--栏目内容标题:--%>
                <%--<input class="easyui-textbox" name="clnftltle" data-options="validType:'length[1,60]',prompt:'请输入栏目内容标题'"/>--%>
