...
<table id="whgdg" title="资料上传管理" class="easyui-datagrid" style="display: none"
       data-options="fit:true, striped:true, rownumbers:true, fitColumns:true, singleSelect:true, checkOnSelect:true, selectOnCheck:true, pagination:true, toolbar:'#whgdg-tb', url:'${basePath}/admin/infoupload/seletup?uptype=${refid}'">
    <thead>
    <tr>
        <th data-options="field:'upname', width:60">标题</th>
        <th data-options="field:'uplink', width:80">地址</th>
...
<div id="whgdg-tb" style="display: none;">
    <div class="whgd-gtb-btn">
        <%-- <shiro:hasPermission name="${resourceid}:add">--%><a href="javascript:void(0)" class="easyui-linkbutton"
                                                                  iconCls="icon-add" onclick="addzysc();"
                                                                  data-options="size:'small'">添加</a><%--</shiro:hasPermission>--%>
        <a href="javascript:void(0)" class="easyui-linkbutton whgff-but-clear" iconCls="icon-undo"
           onclick="WhgComm.editDialogClose()" data-options="size:'small'">返 回</a>
    </div>
</div>
<!-- 表格操作工具栏-END -->
...
<script>
     ...
     */
    function addzysc() {
        WhgComm.editDialog('${basePath}/admin/infoupload/uploads/view/add?refid=' +${refid});
    }
    ...
    function doEdit(idx) {
        ...
        WhgComm.editDialog('${basePath}/admin/infoupload/uploads/view/edit?upid=' + curRow.upid);
    }
    /**
     * 删除
     */
    function delup(idx) {
        ...
        $.messager.confirm('确认对话框', '您确认要删除吗？', function (r) {
            if (r) {
                $.ajax({
                    type: "POST",
                    url: getFullUrl('/admin/infoupload/deluploda'),
    ...
    function checktype(idx) {
        ...
        $.messager.confirm('确认对话框', '确定要更改状态？', function (r) {
            if (r) {
                $.ajax({
                    type: "POST",
                    url: getFullUrl("/admin/infoupload/upstate"),
