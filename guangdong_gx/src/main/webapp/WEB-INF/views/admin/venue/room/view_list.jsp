...
<body>
    ...
    <div id="whgdg-opt" style="display: none;">
        ...
        <shiro:hasPermission name="${resourceid}:edit">
            ...
            <a plain="true" method="whgListTool.setSmsGroup" validFun="whgListTool.optValid4EditState">设置短信组</a>
        </shiro:hasPermission>
        ...
        <shiro:hasPermission name="${resourceid}:retime">
            <a plain="true" method="whgListTool.retime" validFun="whgListTool.retimeVfn">时段管理</a>
        </shiro:hasPermission>
        ...
        <%--<shiro:hasPermission name="${resourceid}:reserve">
            <a plain="true" method="whgListTool.reserve" validKey="state" validVal="4,6">订单管理</a>
        </shiro:hasPermission>--%>
...
<script>
    ...
    $(function(){
        ...
        WhgComm.search('#whgdg', '#tb', '${basePath}/admin/venue/room/srchList4p?pageType=${pageType}');
    });
    ...
    Gridopts.prototype.retime = function(idx){
        var row = this.__getGridRow(idx);
        WhgComm.editDialog( '${basePath}/admin/venue/roomtime/view?roomid='+row.id+'&hasfees='+row.hasfees+'&roomtitle='+ encodeURI(row.title) );
    };

    Gridopts.prototype.resource = function(idx) {
        var row = this.__getGridRow(idx);
        //var cultid = $('#cultid').combobox('getValue');
        //WhgComm.editDialog('${basePath}/admin/resource/view/list?reftype=3&refid=' + row.id+'&cultid='+cultid);
        WhgComm.editDialog('${basePath}/admin/mass/resmanage/view/list?refid=' + row.id+'&reftype=3');
    };
    ...
    Gridopts.prototype.setSmsGroup = function(idx){
        var row = this.__getGridRow(idx);
        var url = '${basePath}/admin/yunwei/aliysmsgroup/refuse/view/edit?enttype=2&entid='+row.id;
