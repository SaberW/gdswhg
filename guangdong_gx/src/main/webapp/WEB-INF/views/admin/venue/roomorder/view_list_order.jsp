...
<body>
<table id="whgdg" class="easyui-datagrid" title="活动室预定管理" style="display: none;"
    ...
    <tr>
        ...
        <th data-options="sortable:false, field:'venTitle' ">场馆</th>
        <th data-options="sortable:false, field:'roomTitle' ">活动室</th>
        ...
        <th data-options="sortable: true, field:'ordercontact'">联系人</th>
        <th data-options="sortable: true, field:'ordercontactphone' ">联系电话</th>
        <th data-options="sortable: true, field:'crtdate', formatter:WhgComm.FMTDateTime ">下单时间</th>
        <th data-options="sortable: true, field:'state', formatter:orderState ">状态</th>
        <th data-options="sortable: true, field:'hasfees', formatter:hasfeesFmt ">状态</th>
...
<div id="tb" style="display: none;">
    <div>
        ...
        <input class="easyui-combobox" name="roomid" id="roomid" prompt="请选择活动室" data-options="editable:false,width:150" />

        <select class="easyui-combobox" name="state" prompt="请选择状态" panelHeight="auto" limitToList="true"
                data-options="width:110, value:'', valueField:'id', textField:'text',
                    data:[{id:'0',text:'申请预定'},
                    {id:'1',text:'取消申请'},
                    {id:'2',text:'审核拒绝'},
                    {id:'3',text:'预定成功'}]">
        </select>
        <input class= "easyui-textbox" name="ordercontactphone" data-options="width:100" prompt="联系电话"/>
        <input class= "easyui-datebox" name="startDay" data-options="width:100" prompt="开始日期"/>
        至 <input class= "easyui-datebox" name="endDay" data-options="width:100" prompt="结束日期"/>
...
<div id="whgdg-opt" style="display: none;">
    ...
    <shiro:hasPermission name="${resourceid}:yescheck">
        <a plain="true" validKey="state" validVal="0" method="whgListTool.yesOrder">审核通过</a>
    </shiro:hasPermission>
    <shiro:hasPermission name="${resourceid}:unyescheck">
...
<script>
    function orderState(v){
        switch (v){
            case 0 : return '<i style="color: red;">申请预定</i>';
            case 1 : return '取消申请';
            case 2 : return '审核拒绝';
            case 3 : return '预定成功';
            default: return v;
    ...
    $(function () {
        WhgComm.initPMS({
            ...
            venEid:'venid', venValue:'',moreVen:false,
            roomEid:'roomid', roomValue:''
        });

        WhgComm.search('#whgdg', '#tb', '${basePath}/admin/venue/roomorder/srchList4pOrder');
    });
    ...
    Gridopts.prototype.yesOrder = function(idx){
        var row = this.__getGridRow(idx);
        var mmx = this;
        WhgComm.confirm("确认信息", "确定通过选中的项的申请吗？", function(r){
            //if (r){
            $.messager.progress();
