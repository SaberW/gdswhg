...
<head>
    ...
    <c:choose>
        ...
        <c:otherwise>
            <c:set var="pageTitle" value="场馆列表"></c:set>
        </c:otherwise>
    </c:choose>
...
<body>
    ...
    <table id="whgdg" class="easyui-datagrid" title="${pageTitle}" style="display: none;"
           ...
           data-options="fit:true, striped:true, rownumbers:true, fitColumns:false, singleSelect:true, checkOnSelect:true">
        <thead>
            <tr>
                <th data-options="sortable:false, field:'title' ">名称</th>
                <th data-options="sortable:false, field:'area',formatter:WhgComm.FMTAreaType ">区域</th>
                <th data-options="sortable:false, field:'address' ">地址</th>
                <th data-options="sortable:false, field:'etype', formatter:WhgComm.FMTVenueType ">分类</th>
    ...
    <div id="tb" style="display: none;">
        ...
        <div class="whgdg-tb-srch">
            ...
            <input class="easyui-textbox" name="title" prompt="请输入场馆名称" data-options="width:200"/>
    ...
    <div id="whgdg-opt" style="display: none;">
        ...
        <shiro:hasPermission name="${resourceid}:ziyuan">
            ...
            <%--<a plain="true" method="whgListTool.resource2" validFun="whgListTool.optValid4PageTypeState">资源管理2</a>--%>
        </shiro:hasPermission>
...
<script>
    ...
    $(function(){
        ...
        WhgComm.search('#whgdg', '#tb', '${basePath}/admin/venue/srchList4p?pageType=${pageType}');
    });

    var whgListTool = new Gridopts({pageType: "${pageType}"});

    /*Gridopts.prototype.resource2 = function (idx) {
        var row = this.__getGridRow(idx);
        var cultid = $('#cultid').combobox('getValue');
        WhgComm.editDialog('${basePath}/admin/resource/view/list?reftype=2&refid=' + row.id+"&cultid="+cultid);
    };*/

    Gridopts.prototype.resource = function (idx) {
        var row = this.__getGridRow(idx);
        WhgComm.editDialog('${basePath}/admin/mass/resmanage/view/list?refid=' + row.id+'&reftype=2');
    };
