...
<head>
    ...
    <c:choose>
        <c:when test="${type eq 'edit'}">
            <c:set var="pageTitle" value="数字展馆编辑列表"></c:set>
            <c:set var="pageValue" value="1"></c:set>
        </c:when>
        <c:when test="${type eq 'check'}">
            <c:set var="pageTitle" value="数字展馆审核列表"></c:set>
            <c:set var="pageValue" value="9"></c:set>
        </c:when>
        <c:when test="${type eq 'publish'}">
            <c:set var="pageTitle" value="数字展馆发布列表"></c:set>
        ...
        <c:otherwise>
            <c:set var="pageTitle" value="数字展馆列表"></c:set>
        </c:otherwise>
    </c:choose>
...
<div id="whgdg-tb" style="display: none;">
    ...
    <div class="whgdg-tb-srch">
        ...
        <input class="easyui-textbox" name="title" prompt="请输入数字展馆名称" data-options="width:120">
        <c:choose>
            <c:when test="${type eq 'edit'}">
                <select class="easyui-combobox" style="width: 110px;" name="state" prompt="请选择状态" panelHeight="auto"
                        limitToList="true"
                        data-options="editable:false,width:120, value:'${pageValue}', valueField:'id', textField:'text', data:WhgComm.getBizState()"></select>
            </c:when>
            <c:when test="${type eq 'del'}"></c:when>
            <c:otherwise>
                <select class="easyui-combobox" style="width: 110px;" name="state" prompt="请选择状态" panelHeight="auto"
                        limitToList="true"
                        data-options="editable:false,width:120, value:'${pageValue}', valueField:'id', textField:'text', data:WhgComm.getBizState('1')"></select>
            </c:otherwise>
        </c:choose>
...
<div id="whgdg-opt" style="display: none;">
    ...
    <shiro:hasPermission name="${resourceid}:view"> <a href="javascript:void(0)" class="easyui-linkbutton" plain="true"
                                                       method="view"> 查看</a></shiro:hasPermission>
    <shiro:hasPermission name="${resourceid}:edit"> <a href="javascript:void(0)" class="easyui-linkbutton" plain="true"
                                                       validFun="validEdit" method="edit"> 编辑</a></shiro:hasPermission>
    <shiro:hasPermission name="${resourceid}:ziyuan"> <a href="javascript:void(0)" class="easyui-linkbutton"
                                                         plain="true" validFun="validResource" method="resource">
        展品管理</a></shiro:hasPermission>
    <shiro:hasPermission name="${resourceid}:checkgo"> <a href="javascript:void(0)" class="easyui-linkbutton"
                                                          plain="true" validFun="validCheckGo" method="checkgo">
        提交审核</a></shiro:hasPermission>
    <shiro:hasPermission name="${resourceid}:checkon"> <a href="javascript:void(0)" class="easyui-linkbutton"
                                                          plain="true" validFun="validCheckOn" method="checkon">
        审核通过</a></shiro:hasPermission>
    <shiro:hasPermission name="${resourceid}:checkoff"> <a href="javascript:void(0)" class="easyui-linkbutton"
                                                           plain="true" validFun="validCheckOff" method="checkoff">
        审核打回</a></shiro:hasPermission>
    <shiro:hasPermission name="${resourceid}:publish"> <a href="javascript:void(0)" class="easyui-linkbutton"
                                                          plain="true" validFun="validPublish" method="publish">
        发布</a></shiro:hasPermission>
    <shiro:hasPermission name="${resourceid}:publishoff"><a href="javascript:void(0)" class="easyui-linkbutton"
                                                            plain="true" validFun="validPublishOff" method="publishoff">撤销发布</a></shiro:hasPermission>

    <shiro:hasPermission name="${resourceid}:recovery"> <a href="javascript:void(0)" class="easyui-linkbutton"
                                                           plain="true" validFun="validRecycle"
                                                           method="del">回收</a></shiro:hasPermission>
    <shiro:hasPermission name="${resourceid}:undel"> <a href="javascript:void(0)" class="easyui-linkbutton" plain="true"
                                                        validFun="validUndel"
                                                        method="undel">撤销回收</a></shiro:hasPermission>
    <shiro:hasPermission name="${resourceid}:del"> <a href="javascript:void(0)" class="easyui-linkbutton" plain="true"
                                                      validFun="validDel" method="del">删除</a></shiro:hasPermission>
    ...
    <form id="whgff" class="whgff" method="post">
        <div class="whgff-row">
            ...
            <div class="whgff-row-input" style="width: 70%">
                <input class="easyui-datetimebox statemdfdate" name="optTime" style="width:200px; height:32px"
...
<script>
    ...
    $(function () {
        ...
        WhgComm.search('#whgdg', '#whgdg-tb', '${basePath}/admin/digitalexhibition/srchList4p?type=${type}');
    });
     ...
     */
    function add() {
        WhgComm.editDialog('${basePath}/admin/digitalexhibition/view/add');
    }
    ...
    function edit(idx) {
        var row = $("#whgdg").datagrid("getRows")[idx];
        WhgComm.editDialog('${basePath}/admin/digitalexhibition/view/add?pageType=1&id=' + row.id);
    }
    ...
    function view(idx) {
        var row = $("#whgdg").datagrid("getRows")[idx];
        WhgComm.editDialog('${basePath}/admin/digitalexhibition/view/add?pageType=2&id=' + row.id);
    }
    ...
    function publish(idx) {
        ...
        $('#whgwin-add').dialog({
            ...
            onOpen: function () {
                ...
                $('#whgff').form({
                    url: '${basePath}/admin/digitalexhibition/updstate',
    ...
    function __updStateSend(ids, formstates, tostate) {
        ...
        $.post('${basePath}/admin/digitalexhibition/updstate', params, function (data) {
            $("#whgdg").datagrid('reload');
     ...
     *!/
     ...
     if (r){
     $.messager.progress();
     $.post('${basePath}/admin/digitalexhibition/del', {id: row.id}, function(data){
     $("#whgdg").datagrid('reload');
     if (!data.success || data.success != "1"){
     $.messager.alert("错误", data.errormsg||'操作失败', 'error');
     }
    ...
    function undel(idx) {
        ...
        WhgComm.confirm("确认信息", "确定要还原选中的项吗？", function () {
            $.messager.progress();
            $.post('${basePath}/admin/digitalexhibition/undel', {id: row.id, delstate: 0}, function (data) {
                $("#whgdg").datagrid('reload');
                if (!data.success || data.success != "1") {
                    $.messager.alert("错误", data.errormsg || '操作失败', 'error');
                }
    ...
    function resource(idx) {
        var row = $("#whgdg").datagrid("getRows")[idx];
        WhgComm.editDialog('${basePath}/admin/mass/resmanage/view/list?enttypes=1&refid=' + row.id + '&reftype=40');
    }
