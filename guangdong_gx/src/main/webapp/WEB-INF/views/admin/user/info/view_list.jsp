...
<%@ taglib prefix="shiro" uri="http://shiro.apache.org/tags" %><% request.setAttribute("resourceid", request.getParameter("rsid")); %>
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
...
<div id="whgdg-tb" style="display: none;">
    <div class="whgdg-tb-srch">
        ...
        <input class="easyui-textbox" name="email" style="width: 150px;"
               data-options="prompt:'请输入邮箱',width:100,validType:'length[1,60]'"/>
...
<div id="whgdg-opt" style="display: none;">
    ...
    <shiro:hasPermission name="${resourceid}:isrealname"><a href="javascript:void(0)" class="easyui-linkbutton" plain="true" validKey="isrealname" validVal="3" method="doCheck">实名审核</a></shiro:hasPermission>
    <shiro:hasPermission name="${resourceid}:setuserphone"><a href="javascript:void(0)" class="easyui-linkbutton" plain="true" method="setuserphone">更改手机</a></shiro:hasPermission>
    ...
    function setuserphone(idx){
        ...
        var optDialog =$('<div></div>').dialog({
            ...
            onOpen: function(){
                ...
                content.append('手机号码：<input class= "easyui-textbox" data-options="width:200,required:true, validType:\'length[0,11]\'"/>');
                $.parser.parse(content);
                content.find(".easyui-textbox:eq(0)").textbox('setValue', row.phone);
            },
            buttons: [{
                text:'确认',
                iconCls: 'icon-ok',
                handler:function(){
                    $.messager.progress();
                    var valid = optDialog.find(".easyui-textbox:eq(0)").textbox('isValid');
                    if (!valid) {
                        $.messager.progress('close');
                        return;
                    }
                    var phone = optDialog.find(".easyui-textbox:eq(0)").textbox('getValue');

                    $.ajax({
                        url: '${basePath}/admin/user/info/setuserphone',
                        data: {id: row.id, phone: phone},
                        type: 'post',
                        dataType: 'json',
