...
<head>
    ...
    <c:choose>
        <c:when test="${not empty id and not empty targetShow}">
            <c:set var="pageTitle" value="查看节目单"></c:set>
        </c:when>
        <c:when test="${not empty id}">
            <c:set var="pageTitle" value="编辑节目单"></c:set>
        </c:when>
        <c:otherwise>
            <c:set var="pageTitle" value="添加节目单"></c:set>
        </c:otherwise>
    </c:choose>
...
<form id="whgff" class="whgff" method="post" >
    ...
    <div class="whgff-row">
        ...
        <div class="whgff-row-input"><input class="easyui-textbox" name="title" value="${play.title}"
                                            style="width:500px; height:32px"
                                            data-options="required:true, validType:'length[1,60]'"></div>
    </div>

    <div class="whgff-row">
        <div class="whgff-row-label">类型：</div>
        <div class="whgff-row-input"><input class="easyui-textbox" name="etype" value="${play.etype}"
                                            style="width:500px; height:32px" data-options="validType:'length[1,60]'">
        </div>
    </div>

    <div class="whgff-row">
        <div class="whgff-row-label">参演人员：</div>
        <input class="easyui-numberbox" name="person" value="${play.person}" style="width:300px; height:32px" data-options="min:0,max:1000000, prompt:'请输入参演人员'"> 人
    </div>
    <div class="whgff-row">
        <div class="whgff-row-label">时长：</div>
        <input class="easyui-numberbox" name="time" value="${play.time}" style="width:300px; height:32px" data-options="min:0,max:1000000, prompt:'请输入时长'"> 分钟
    </div>
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        //定义表单提交
        var url = id ? "${basePath}/admin/show/jmd/edit" : "${basePath}/admin/show/jmd/add";
        frm.form({
            url: url,
            novalidate: true,
