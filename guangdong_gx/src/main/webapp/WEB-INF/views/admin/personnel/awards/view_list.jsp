...
<body>
<!-- 表格 -->
<table id="whgdg" title="获奖情况列表" class="easyui-datagrid" style="display: none"
       data-options="fit:true, striped:true, rownumbers:true, fitColumns:true, singleSelect:false, checkOnSelect:true, selectOnCheck:true, pagination:true, toolbar:'#whgdg-tb', url:'${basePath}/admin/per/awards/srchList4p?entid=${entid}'">
    <thead>
    <tr>
        <th data-options="field:'title',width:80">奖项</th>
        <th data-options="field:'issuedate', width:80,formatter:WhgComm.FMTDate">获奖时间</th>
...
<script>
     ...
     */
    function add(){
        WhgComm.editDialog('${basePath}/admin/per/awards/view/edit?entid=${entid}');
    }
    ...
    function edit(idx){
        ...
        WhgComm.editDialog('${basePath}/admin/per/awards/view/edit?entid=${entid}&id='+row.id);
    }
    ...
    function view(idx){
        var row = $("#whgdg").datagrid("getRows")[idx];
        WhgComm.editDialog('${basePath}/admin/per/awards/view/edit?targetShow=2&id='+row.id);
    }
    ...
    function del(idx){
        ...
        $.messager.confirm("确认信息", confireStr, function(r){
            if (r){
                $.messager.progress();
                $.post('${basePath}/admin/per/awards/del', {id: row.id}, function(data){
