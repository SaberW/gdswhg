...
<body>
    ...
    <table id="whgdg" title="活动管理-场次列表(${act.name})" class="easyui-datagrid"
           data-options="fit:true,
                                         ...
                                         pagination:true,
                                         toolbar:'#whgdg-tb',
                                         url:'${basePath}/admin/activity/getScreenings?activityId=${act.id}'
     ">
        <thead>
            <tr>
    ...
    <script type="text/javascript">
                    ...
                    buttons:[
                        {
                            ...
                            handler:function () {
                                ...
                                if (isvalide == '3' && Number($("#seats").numberspinner('getValue')) > Number("${act.ticketnum}")) {
                                	$.messager.alert("错误", '新增座位数不允许大于活动场次总座位数!', 'error');
                                	return false;
                                }
                                $.post("${basePath}/admin/activity/addScreenings?actId="+actid,$("#whgff").serialize(),function (data) {
                                    $("#dd").dialog('close');
                                    if(1 == data.success){
                    ...
                    buttons:[
                        {
                            ...
                            handler:function () {
                                ...
                                if (isvalide == '3' && Number($("#seats").numberspinner('getValue')) > Number("${act.ticketnum}")) {
                                    $.messager.alert("错误", '座位数不允许大于活动场次总座位数!', 'error');
                                    return false;
                                }
                                $.post("${basePath}/admin/activity/updateScreenings",$("#whgff").serialize(),function (data) {
                                    $("#dd").dialog('close');
                                    if(1 == data.success){
                                        $("#whgdg").datagrid('reload');
                                    }else{
        ...
        function createSeats(seats) {
            ...
            subObject1.appendTo(object);
            var subObject2 = $('<div class="whgff-row-input" style="width: 70% !important;">');
            subObject2.append('<input class="easyui-numberspinner" id="seats" name="seats" style="width:200px; height:32px" data-options="validType:\'length[1,5]\', required:true"  value="' + seats + '">');
            subObject2.appendTo(object);
            object.appendTo("#whgff");
            $.parser.parse("#whgff");
        }
        ...
        function updateScreeningsStatus(rowIndex,status) {
            ...
            param += "&seats=" + seats;
            param += "&state=" + status ;
            $.getJSON("${basePath}/admin/activity/updateScreenings?" + param,function (data) {
                if(1 == data.success){
                    $("#whgdg").datagrid('reload');
                }else{
                	$.messager.alert("错误", data.errormsg, 'error');
