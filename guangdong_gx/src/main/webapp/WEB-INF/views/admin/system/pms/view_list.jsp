...
<table id="whgdg" title="权限组管理" class="easyui-datagrid" style="display: none"
       data-options="fit:true, striped:true, rownumbers:true, fitColumns:true, singleSelect:true, pagination:true, toolbar:'#whgdg-tb', url:'${basePath}/admin/system/pms/srchList4p?sysflag=bizmgr'">
    <thead>
    <tr>
...
<div id="whgdg-tb" style="display: none;">
    ...
    <div class="whgd-gtb-btn">
            <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-add" onclick="doAdd('bizmgr');">添加权限组</a>
    </div>
    </shiro:hasPermission>

    <div class="whgdg-tb-srch">
        <form id="whgdg-tb-srch-form">
            <input class="easyui-combotree" style="width: 200px;" id="cultid" name="cultid" data-options="prompt:'请选择文化馆', editable:false"/>
            ...
            <c:if test="${isSuperMan}">
            <input class="easyui-combobox" style="width: 170px;" name="sysflag" data-options="prompt:'请选择权限组所属系统', editable:false, valueField:'id', textField:'text', data:getConsoleSystems()"/>
            </c:if>
...
<div id="whgdg-opt" style="display: none;">
    ...
    <shiro:hasPermission name="${resourceid}:syzd"><a href="javascript:void(0)" class="easyui-linkbutton" plain="true" validFun="vaildScopeSite" method="doScopeSite">适用站点</a></shiro:hasPermission>
...
<script type="text/javascript">
    ...
    function doScopeSite(idx) {
        var curRow = $('#whgdg').datagrid('getRows')[idx];
        WhgComm.openDialog4size('权限组[<span style="color:#FF9800;">'+curRow.name+'</span>]适应站点配置', '${basePath}/admin/system/pms/view/site?id='+curRow.id+'&pmsName='+encodeURIComponent(curRow.name), 800, 600, {cancelBtnText:'关  闭',closeFun: function(){
    ...
    /** 加载树 */
    $(function(){
        WhgCommAreaTree.initAreaCombotree({
            eleId: 'cultid',
            loadCult: true,
            canSelectArea: false
        });
