...
<form id="whgff" class="whgff" method="post">
        ...
        <div class="whgff-row-input">
            <c:choose>
                <c:when test="${type == 'copy'}">
                    <input class="easyui-combobox" style="width: 300px; height:32px" name="cultid"  data-options="required:true, prompt:'请选择文化馆', valueField:'id', textField:'text', data:WhgComm.getMgrCultsAndChild()"/>
                </c:when>
                <c:otherwise>
                    <input class="easyui-combobox" style="width: 300px; height:32px" name="cultid" value="${role.cultid}" data-options="required:true, prompt:'请选择文化馆', valueField:'id', textField:'text', data:WhgComm.getMgrCultsAndChild()"/>
                </c:otherwise>
            </c:choose>
        </div>
    </div>
    ...
    <div class="whgff-row">
        ...
        <div class="whgff-row-input">
            <c:choose>
                <c:when test="${type == 'copy'}">
                <input class="easyui-textbox" name="name" style="width:300px; height:32px" data-options="prompt:'请输入角色名称', required:true, validType:'length[3,30]'"></div>
                </c:when>
                <c:otherwise>
                <input class="easyui-textbox" name="name" value="${role.name}" style="width:300px; height:32px" data-options="prompt:'请输入角色名称', required:true, validType:'length[3,30]'"></div>
                </c:otherwise>
            </c:choose>
    </div>
...
<script type="text/javascript">
    ...
    function chioceAll(checked){
        if(checked){
            $("input[type='checkbox'][cctype='true'][name='pms']").prop('checked', true);
            $("input[type='checkbox'][cctype='true'][name='pmsAll']").prop('checked', true);
        }else{
            $("input[type='checkbox'][cctype='true'][name='pms']").prop('checked', false);
            $("input[type='checkbox'][cctype='true'][name='pmsAll']").prop('checked', false);
        }
    }
