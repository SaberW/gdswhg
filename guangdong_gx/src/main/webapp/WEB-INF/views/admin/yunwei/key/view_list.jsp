...
<head>
    ...
    <c:choose>
        ...
        <c:when test="${classify == 36}">
            <c:set var="pageTitle" value="资讯关键字配置"></c:set>
        </c:when>
        <c:when test="${classify == 27}">
            <c:set var="pageTitle" value="培训师资关键字配置"></c:set>
        </c:when>
        <c:when test="${classify == 16}">
            <c:set var="pageTitle" value="在线课程关键字配置"></c:set>
        </c:when>
        <c:when test="${classify == 20}">
            <c:set var="pageTitle" value="文艺辅材关键字配置"></c:set>
        </c:when>
        <c:when test="${classify == 23}">
            <c:set var="pageTitle" value="展览展示关键字配置"></c:set>
        </c:when>
        <c:when test="${classify == 21}">
            <c:set var="pageTitle" value="文艺演出关键字配置"></c:set>
        </c:when>
        <c:when test="${classify == 38}">
            <c:set var="pageTitle" value="供需培训关键字配置"></c:set>
        </c:when>
        <c:when test="${classify == 25}">
            <c:set var="pageTitle" value="文艺专家关键字配置"></c:set>
        </c:when>
        <c:otherwise>
...
<table id="whgdg" title="${pageTitle}" class="easyui-datagrid" style="display: none"
       data-options="fit:true, striped:true, rownumbers:true, fitColumns:true, singleSelect:false, checkOnSelect:true, selectOnCheck:true, pagination:true, toolbar:'#whgdg-tb', queryParams:{cultid:WhgComm.getMgrCultsFirstId()}, url:'${basePath}/admin/yunwei/key/srchList4p?type=${classify}'">
    <thead>
    <tr>
        ...
        $('#whgwin-add').dialog({
            ...
            onOpen : function () {
                ...
                if (row){
                    $('#whgff').form('load', row);
                    $('#whgff').form({
                        url : '${basePath}/admin/yunwei/key/edit?type=${classify}',
                        onSubmit : function(param) {
                            var isValid = $(this).form('enableValidation').form('validate');
                            if(isValid){
                                $.messager.progress();
