...
<head>
	...
	<script>
        ...
        function _goAdd(){
            ...
            if(dir == "root" || dir.indexOf('/') < 0){
                $.messager.alert('警告', '请先创建目录后再上传，不能直接上传到根目录下！', 'warning');
                return false;
			}
        ...
        function _cdir(){
            ...
            $('#ff').form({
                ...
                onSubmit : function(param) {
                    ...
                    param.cultid = $('#cultid').combobox('getValue');
        ...
        /**
		 * 切换文化馆时，需要重置input.dir的值
         * @param newVal
         * @param oldVal
         */
        function changeCult(newVal, oldVal){
            $('#whgdg-tb').find('input[name="dir"]').val("root");
		}
         ...
         * @param rowData
         */
		function dblClickRow(rowIdx, rowData){
            if(rowData.etag == 'predir'){//返回上级目录
                doLSP(rowIdx);
            }else if(rowData.etag == 'dir'){//返回下级目录
                doLS(rowIdx);
            }
            return false;
		}
...
<table id="whgdg" title="音视频管理" class="easyui-datagrid" style="display: none"
	   data-options="fit:true, striped:true, rownumbers:true, fitColumns:true, singleSelect:false, checkOnSelect:true, selectOnCheck:true, pagination:false, toolbar:'#whgdg-tb',
	   queryParams: {dir:'root', cultid:WhgComm.getMgrCultsFirstId()}, url:'${basePath}/admin/video/srchPagging', onDblClickRow: dblClickRow">
	<thead>
	<tr>
		<th data-options="field:'ck', checkbox:true"></th>
		<th data-options="field:'etag', width:80, formatter:function(val,row,index){return 'dir'==val || 'predir' == val ? '目录':'文件';}">类型</th>
		<th data-options="field:'keyValue', width:160">名称</th>
...
<div id="whgdg-tb" style="display: none;">
	<div class="whgd-gtb-btn">
		<shiro:hasPermission name="${resourceid}:add"> <a size="small" class="easyui-linkbutton" data-options="iconCls:'icon-add'" id="btn_add">上传音视频</a></shiro:hasPermission>
	...
	<div class="whgdg-tb-srch">
        ...
        <input class="easyui-textbox" name="keyname" style="width:300px"
               data-options="prompt:'请输入音视频名称进行查询',validType:'length[1,60]'"/>
		<input class="easyui-combobox" style="width: 200px;" id="cultid" name="cultid" data-options="prompt:'请选择文化馆', editable:false,  value:WhgComm.getMgrCultsFirstId(), valueField:'id', textField:'text', data:WhgComm.getMgrCults(), onChange:changeCult"/>
...
<script type="text/javascript">
    ...
    $(function(){
        //注册添加事件
        $('#btn_add').on('click.wxl', _goAdd); //上传视频
        $('#btn_bdel').on('click.wxl', function(e){e.preventDefault();_updState();});//批量删除
        $('#btn_cdir').on('click.wxl', function(e){e.preventDefault();_cdir();});//创建目录
        $('#btn_refresh').on('click.wxl', function(){
            $('#whgdg-tb').find('.search').click(); //刷新
        });
