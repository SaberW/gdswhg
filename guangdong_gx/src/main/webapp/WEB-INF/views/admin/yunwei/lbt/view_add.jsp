...
<form id="whgff" method="post" class="whgff">
    ...
    <div class="whgff-row">
        ...
        <div class="whgff-row-input">
            <input class="easyui-textbox" name="name" style="width:600px; height:32px" data-options="required:true, validType:'length[0,30]'">
        </div>
    </div>
...
<script>
    ...
    function initCultProvinceCity(){
        var flag = '${param.flag}';//flag=1表示添加区域轮播图
        if(flag == '1'){
            var province = '${param.province}';
            var city = '${param.city}';
            if(province != ''){
                $('.toProvince').show();
                $('#province').textbox('setValue', province);
            }else if(city != ''){
                $('.toCity').show();
                $('#city').textbox('setValue', city);
            }
