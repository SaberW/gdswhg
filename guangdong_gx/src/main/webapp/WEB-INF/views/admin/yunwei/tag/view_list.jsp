...
<head>
    ...
    <c:choose>
        ...
        <c:when test="${classify == 20}">
            <c:set var="pageTitle" value="文艺辅材标签配置"></c:set>
        </c:when>
        <c:when test="${classify == 27}">
            <c:set var="pageTitle" value="培训师资标签配置"></c:set>
        </c:when>
        <c:when test="${classify == 16}">
            <c:set var="pageTitle" value="在线课程标签配置"></c:set>
        </c:when>
        <c:when test="${classify == 38}">
            <c:set var="pageTitle" value="供需培训人员标签配置"></c:set>
        </c:when>
        <c:when test="${classify == 53}">
            <c:set var="pageTitle" value="群文资源标签配置"></c:set>
        </c:when>
        <c:when test="${classify == 2002}">
            <c:set var="pageTitle" value="内供-场馆标签配置"></c:set>
        </c:when>
        <c:otherwise>
...
<table id="whgdg" title="${pageTitle}" class="easyui-datagrid" style="display: none"
       data-options="fit:true, striped:true, rownumbers:true, fitColumns:true, singleSelect:false, checkOnSelect:true, selectOnCheck:true, pagination:true, toolbar:'#whgdg-tb', queryParams:{cultid:WhgComm.getMgrCultsFirstId()}, url:'${basePath}/admin/yunwei/tag/srchList4p?type=${classify}'">
    <thead>
    <tr>
                ...
                $('#whgff').form({
                    ...
                    success : function(data) {
                        ...
                        if(Json.success == "1"){
                            ...
                            $.messager.alert("提示", Json.errormsg || "操作失败");
        ...
        $('#whgwin-add').dialog({
            ...
            onOpen : function () {
                ...
                if (row){
                    $('#whgff').form('load', row);
                    $('#whgff').form({
                        url : '${basePath}/admin/yunwei/tag/edit?type=${classify}',
                        onSubmit : function(param) {
                            var isValid = $(this).form('enableValidation').form('validate');
                            if(isValid){
                                $.messager.progress();
