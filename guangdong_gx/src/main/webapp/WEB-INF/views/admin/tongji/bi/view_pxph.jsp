...
<body class="easyui-layout">
    <div data-options="region:'west',title:'培训范围',collapsible:false,split:true,tools:[{iconCls:'icon-reload', handler:reloadTree}]" style="width:250px;padding:10px;">
        <ul class="easyui-tree" id="mass_type_tree"></ul>
    </div>
    <div data-options="region:'center',title:'培训统计列表'">
        <!-- 表格 -->
        <table id="whgdg" class="easyui-datagrid" style="display: none"
            ...
            <thead>
            <tr>
                <th data-options="field:'title', width:300">培训名称</th>
                <th data-options="field:'tickettotal', width:100, sortable:true">学位数</th>
                <th data-options="field:'ticketorder', width:100, sortable:true">报名数</th>
                <th data-options="field:'ticketvalidtotal', width:100, sortable:true">应签到数</th>
                <th data-options="field:'ticketvalid', width:100, sortable:true">已签到数</th>
                <th data-options="field:'ratebook', width:100, sortable:true">报名率(%)</th>
                <th data-options="field:'ratesign', width:100, sortable:true">签到率(%)</th>
                <th data-options="field:'likes', width:100, sortable:true">点赞数</th>
        ...
        <div id="whgdg-tb" style="display: none;">
            <div class="whgdg-tb-srch">
                ...
                <input class="easyui-textbox" style="width: 200px;" id="traName" name="traName" data-options="prompt:'请输入培训名称', validType:'length[1,32]'" />
            ...
            WhgCommAreaTree.initAreaTree({
                ...
                onSelect: function(node, firstLevel, areaInfo){
                    if(typeof(node.iscult) != 'undefined' && node.iscult == '1'){//选择文化馆
                        $('#cultId').val(node.id);
                        $('#traName').textbox('clear');
                        $('#province').val('');
                        $('#city').val('');
                        $('#area').val('');
                        $('#queryBtn').linkbutton('enable');
                        WhgComm.search('#whgdg', '#whgdg-tb', '${basePath}/admin/tongji/bi/pxph');
                    }else{//选择省市区
                        $('#cultId').val('');
                        //$('#traName').textbox('clear');
