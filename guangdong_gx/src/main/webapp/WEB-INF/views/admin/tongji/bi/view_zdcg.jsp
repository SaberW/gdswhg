...
<div data-options="region:'center',title:'站点场馆统计列表'">
    <!-- 表格 -->
    <table id="whgdg" class="easyui-datagrid" style="display: none"
        ...
        <tr>
            ...
            <th data-options="field:'vens', width:100, sortable:true">场馆总数</th>
            <th data-options="field:'rooms', width:100, sortable:true">活动室总数</th>
            <th data-options="field:'sizearea', width:100, sortable:true">可预订面积(㎡)</th>
            <th data-options="field:'timelong', width:120, sortable:true">可预订时长(分钟)</th>
            <th data-options="field:'booknumbs', width:100, sortable:true">预订次数</th>
            <th data-options="field:'booktimes', width:100, sortable:true">预订时长(分钟)</th>
            <th data-options="field:'ratebook', width:100, sortable:true">预订使用率(%)</th>
        </tr>
        </thead>
    </table>
    <!-- 表格 END -->
...
<script type="text/javascript">
        ...
        WhgCommAreaTree.initAreaTree({
            ...
            onSelect: function(node, firstLevel, areaInfo){
                if(typeof(node.iscult) != 'undefined' && node.iscult == '1'){//选择文化馆
                    ...
                    WhgComm.search('#whgdg', '#whgdg-tb', '${basePath}/admin/tongji/bi/zdcg');
                }else{//选择省市区
                    $('#cultId').val('');
                    //$('#cultName').textbox('clear');
    ...
    function loadSuccess() {
        //添加“合计”列
        var total_timelong = compute("timelong");
        var rate = '0.0';
        if(total_timelong != '0'){
            rate = ((compute("booktimes") / total_timelong) * 100).toFixed(2);
        }
        $('#whgdg').datagrid('appendRow', {
            'name': '<span class="subtotal" style="float: right;">合计</span>',
            'vens': '<span class="subtotal">' + compute("vens") + '</span>',
            'rooms': '<span class="subtotal">' + compute("rooms") + '</span>',
            'sizearea': '<span class="subtotal">' + compute("sizearea") + '</span>',
            'timelong': '<span class="subtotal">' + total_timelong + '</span>',
            'booknumbs': '<span class="subtotal">' + compute("booknumbs") + '</span>',
            'booktimes': '<span class="subtotal">' + compute("booktimes") + '</span>',
            'ratebook': '<span class="subtotal">' + rate + '</span>'
        });
    }
