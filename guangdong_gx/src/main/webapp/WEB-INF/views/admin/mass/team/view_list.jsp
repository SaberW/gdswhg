...
<head>
    ...
    <c:choose>
        ...
        <c:otherwise>
            <c:set var="pageTitle" value="艺术团队列表"></c:set>
        </c:otherwise>
    </c:choose>
...
<table id="whgdg" class="easyui-datagrid" title="${pageTitle}" style="display: none;"
    ...
    <thead>
    <tr>
        <th data-options="sortable:false, width:200, field:'name' ">名称</th>
        ...
        <c:if test="${pageType eq 'listpublish'}">
            <th data-options="sortable:true,width:50, field:'recommend', formatter:function(v){
                return (v && v==1)? '是':'否'
            }">是否推荐</th>
        </c:if>
...
<div id="tb" style="display: none;">
    ...
    <div class="whgdg-tb-srch">
        <input class="easyui-combobox" name="cultid" id="cultid" prompt="请选择文化馆" data-options="editable:true,width:150"/>
        <input class="easyui-combobox" name="deptid" id="deptid" prompt="请选择部门" data-options="editable:true,width:150"/>
        ...
        <input class="easyui-textbox" name="name" prompt="请输入名称" data-options="width:120">
...
<script>
    ...
    $(function(){
        ...
        WhgComm.search('#whgdg', '#tb', '${basePath}/admin/mass/team/srchList4p?pageType=${pageType}');
    });
