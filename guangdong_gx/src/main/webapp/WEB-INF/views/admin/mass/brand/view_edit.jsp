...
<head>
    ...
    <c:choose>
        <c:when test="${pageType eq 'show'}">
            <c:set var="pageTitle" value="文化专题管理-查看"></c:set>
        </c:when>
        <c:when test="${pageType eq 'edit'}">
            <c:set var="pageTitle" value="文化专题管理-编辑"></c:set>
        </c:when>
        <c:otherwise>
            <c:set var="pageTitle" value="文化专题管理-添加"></c:set>
        </c:otherwise>
    </c:choose>
...
<form id="whgff" method="post" class="whgff">
    ...
    <div class="whgff-row">
        <div class="whgff-row-label"><i>*</i>专题背景颜色：</div>
        <div class="whgff-row-input">
            <input class="easyui-textbox" name="bgcolor" value="${info.bgcolor}" style="width:200px; height:32px"
                   data-options="required:true,validType:['length[1,100]'], prompt:'请输入内容'">
            (格式: #ffffff)
        </div>
    </div>--%>
    ...
    <div class="whgff-row">
        <div class="whgff-row-label"><i>*</i>文化专题介绍：</div>
        <div class="whgff-row-input">
    ...
    var formTool = {
        ...
        initVal: function(){
            ...
            var ueConfig = {
                ...
                readonly: me.pageType=='show'
            };
            me.description = UE.getEditor('description', ueConfig);
            me.description.ready(function(){  me.description.setContent($("#value_description").val()) });
