...
<div data-options="region:'center',title:'群文资源'">
        ...
        <table id="whgdg" class="easyui-datagrid" style="display: none"
               data-options="border:0, fit:true, striped:true, rownumbers:true, fitColumns:true, singleSelect:false, pagination:true, toolbar:'#whgdg-tb',selectOnCheck:false">
        ...
        <div id="whgdg-tb" style="display: none;">
            <div class="whgd-gtb-btn" style="display: none;">
            ...
            <div class="whgdg-tb-srch">
                <form id="whgdg-tb-srch-form">
                    ...
                    <a href="javascript:void(0)" id="resdownload" class="easyui-linkbutton" iconCls="icon-large-smartart" onclick="resdownload();">
                        资源导出
                    </a>

                    <shiro:hasPermission name="${resourceid}:checkgo">
                        <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-ok" onclick="doBatchUpd(9);">
                            批量提交审核
                        </a>
                    </shiro:hasPermission>
                    <shiro:hasPermission name="${resourceid}:checkon">
                        <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-ok" onclick="doBatchUpd(2);">
                            批量通过
                        </a>
                    </shiro:hasPermission>
                    <shiro:hasPermission name="${resourceid}:checkoff">
                        <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-no" onclick="doBatchUpd(1);">
                            批量打回
                        </a>
                    </shiro:hasPermission>
                    <shiro:hasPermission name="${resourceid}:publish">
                        <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-ok" onclick="doBatchUpd(6);">
                            批量发布
                        </a>
                    </shiro:hasPermission>
                    <shiro:hasPermission name="${resourceid}:sharelib">
                        <a href="javascript:void(0)" class="easyui-linkbutton addEditResources" style="float: right;margin-right: 10px;display:none;" iconCls="" id="shareLibrary" onclick="doShareLibrary()">分享资源库</a>
                    </shiro:hasPermission>
                </form>
            </div>
        </div>
        ...
        <div id="whgdg-opt" style="display: none;">
            ...
            <shiro:hasPermission name="${resourceid}:edit"><a href="javascript:void(0)" class="easyui-linkbutton addEditResources" plain="true" validFun="validEdit" method="doEdit">编辑</a></shiro:hasPermission>
            <shiro:hasPermission name="${resourceid}:checkgo"><a href="javascript:void(0)" class="easyui-linkbutton addEditResources" plain="true" validFun="validCheckgo" method="doCheckgo">提交审核</a></shiro:hasPermission>
            <shiro:hasPermission name="${resourceid}:checkon"><a href="javascript:void(0)" class="easyui-linkbutton addEditResources" plain="true" validFun="validCheckon" method="doCheckon">审核通过</a></shiro:hasPermission>
            <shiro:hasPermission name="${resourceid}:checkoff"><a href="javascript:void(0)" class="easyui-linkbutton addEditResources" plain="true" validFun="validCheckoff" method="doCheckoff">审核打回</a></shiro:hasPermission>
            <shiro:hasPermission name="${resourceid}:publish"><a href="javascript:void(0)" class="easyui-linkbutton addEditResources" plain="true" validFun="validPublish" method="doPublish">发布</a></shiro:hasPermission>
            <shiro:hasPermission name="${resourceid}:publishoff"><a href="javascript:void(0)" class="easyui-linkbutton addEditResources" plain="true" validFun="validPublishoff" method="doPublishoff">撤销发布</a></shiro:hasPermission>
            <shiro:hasPermission name="${resourceid}:recommend"><a href="javascript:void(0)" class="easyui-linkbutton addEditResources" plain="true" validFun="validRecommend" method="doRecommend">推荐</a></shiro:hasPermission>
            <shiro:hasPermission name="${resourceid}:recommendoff"><a href="javascript:void(0)" class="easyui-linkbutton addEditResources" plain="true" validFun="validRecommendoff" method="doRecommendoff">撤销推荐</a></shiro:hasPermission>
            <shiro:hasPermission name="${resourceid}:recovery"><a href="javascript:void(0)" class="easyui-linkbutton addEditResources" plain="true" validFun="validRecovery" method="doRecovery">回收</a></shiro:hasPermission>
            <shiro:hasPermission name="${resourceid}:undel"><a href="javascript:void(0)" class="easyui-linkbutton addEditResources" plain="true" validFun="validUndel" method="doUndel">撤销回收</a></shiro:hasPermission>
            <shiro:hasPermission name="${resourceid}:del"><a href="javascript:void(0)" class="easyui-linkbutton addEditResources" plain="true" validFun="validDel" method="doDel">删除</a></shiro:hasPermission>
        </div>
        <!-- 操作按钮-END -->
    </div>
...
<script type="text/javascript">
    ...
    function resdownload(){
        $("#resdownload").removeAttr("onclick");
        //选中的记录数
        var rows = $('#whgdg').datagrid('getChecked');
        if (rows.length < 1) {
            $.messager.alert('提示', '请选中要导出的资源！', 'warning');
            $("#resdownload").attr("onclick","resdownload()");
            return;
        }
        var _ids = "";
        var spt = "";
        for (var i = 0; i < rows.length; i++) {
            _ids += spt + rows[i].resurl.replace("http://szwhg-gds-admin.oss-cn-shanghai.aliyuncs.com/","");
            spt = ',';
        }
        window.location.href= basePath+'/admin/mass/libres/download?key='+_ids
        $("#resdownload").attr("onclick","resdownload()");
    }
    ...
    function doBatchUpd(state) {
        ...
        for (var i = 0; i < rows.length; i++) {
            _ids += spt + rows[i].id;
            _libids += spt+rows[i].libid;
            spt = ',';
        }
        //提交审核
        if(state==9){
            WhgComm.confirm("确认信息", "确定要批量提交选中的项吗？", function(){
                batchSubmit(_libids, _ids, state);
            })
        //审核通过
        }else if(state==2){
            WhgComm.confirm("确认信息", "确定要批量审核选中的项吗？", function(){
                batchSubmit(_libids, _ids, state);
            })
        //审核打回
        }else if(state==1){
            WhgComm.confirm("确认信息", "确定要批量打回选中的项吗？", function(){
                batchSubmit(_libids, _ids, state);
            })
        //发布
        }else if(state==6){
            WhgComm.confirm("确认信息", "确定要批量发布选中的项吗？", function(){
                batchSubmit(_libids, _ids, state);
            })
        }
    }
    ...
    /** 修改资源业务状态 */
    function batchSubmit(libid, id, state) {
        $.ajax({
            type: "POST",
            cache: false,
            url: getFullUrl('/admin/mass/libres/batchSubmit'),
            data: {"libid":libid, "id":id, "state": state},
            success: function(Json){
                if(Json && Json.success == '1'){
                    $('#whgdg').datagrid('reload');
    ...
    function initMassTypeTree(){
        $('#mass_type_tree').tree({
            url: '${basePath}/admin/mass/library/srchlibs',
            animate: true,
            lines: true,
            ...
            onSelect: function(node){
                ...
                $.getJSON("/admin/mass/library/findLibrary?id="+node.id, function (data) {
                    ...
                    WhgComm.search('#whgdg', '#whgdg-tb', '${basePath}/admin/mass/libres/srchList4p?ptype=${param.ptype}&delstate=${param.delstate}');
                });
            }
        });
    }
    ...
    $(function () {
        ...
        //查询
        WhgComm.search('#whgdg', '#whgdg-tb', '${basePath}/admin/mass/libres/srchList4p?ptype=${param.ptype}');
    });
</script>
<!-- script END -->
</body>
