...
<head>
    ...
    <c:choose>
        <c:when test="${not empty id and not empty targetShow}">
            <c:set var="pageTitle" value="直播管理-查看直播"></c:set>
        </c:when>
        <c:when test="${not empty id}">
            <c:set var="pageTitle" value="直播管理-编辑直播"></c:set>
        </c:when>
    </c:choose>
            ...
            edittraTime: {
                validator: function(value, param){
                    ...
                    if(_type === 0){//0-starttime
                        ...
                        if (_ptime.substring(0, 11) != value.substring(0, 11)) {
                            //直播时间段必须在同一天
                ...
                validator: function(value, param){
                    ...
                    if (parseInt(mutle) === 0) {
                        ...
                        if (_type === 0) {
                            var sdVal = false;
                            var mustsignup = $("input[name='mustsignup']:checked").val();
                            if (mustsignup == '1') {//需要报名
                                sdVal = $("#whgff").find("input[name='enrollendtime']").val();
                                var d1 = WhgComm.parseDateTime(sdVal);
                            ...
                            var sdVal = $('#' + _refid).datebox('getValue');
                            var d1 = WhgComm.parseDateTime(sdVal);
                            var d2 = WhgComm.parseDateTime(value + " 23:59:59");
                            return d2.getTime() > d1.getTime();
                        }
...
<form id="whgff" method="post" class="whgff">
    ...
    <div class="whgff-row">
        ...
        <div class="whgff-row-input">
            <div class="radio radio-primary whg-js-data" value="${whgTra.mustsignup}" name="mustsignup" value="" js-data='[{"id":"0","text":"否"},{"id":"1","text":"是"}]'></div>
        </div>
    </div>
    ...
    <c:if test="${not empty course }">
        <c:forEach items="${course}" var="item" varStatus="s" end="0">
            <div class="whgff-row single_edit" >
                ...
                <div class="whgff-row-input">
                    <input class="easyui-datetimebox sin_starttime_edit" id="sin_starttime_edit" name="sin_starttime"
                           value='<fmt:formatDate value="${item.starttime}" pattern="yyyy-MM-dd HH:mm:ss"/>'
                           style="width:200px; height:32px" data-options="validType:'sinedit_Time[\'enrollendtime\']'"/>至
                    <input class="easyui-datetimebox sin_endtime_edit" id="sin_endtime_edit" name="sin_endtime"
                           value='<fmt:formatDate value="${item.endtime}" pattern="yyyy-MM-dd HH:mm:ss"/>'
                           style="width:200px; height:32px"
                           data-options="validType:['sinedit_Time[\'sin_starttime_edit\']', 'oneDay[\'sin_starttime_edit\']']"/>
                </div>
            </div>
        </c:forEach>
    </c:if>
    ...
    <div class="whgff-row fixed">
        ...
        <div class="whgff-row-input">
            <input class="easyui-timespinner _coursestarttime" name="fixedstarttime"
                   value="<fmt:formatDate value="${whgTra.fixedstarttime}" pattern="HH:mm"/>"
                   style="width:200px; height:32px"/>至
            <input class="easyui-timespinner _courseendtime" name="fixedendtime"
                   value="<fmt:formatDate value="${whgTra.fixedendtime}" pattern="HH:mm"/>"
                   style="width:200px; height:32px"/>
        </div>
    </div>
    ...
    <%--<div class="whgff-row">
            ...
            <div class="whgff-row-input">
                <input class="easyui-combobox" style="width:245px; height:32px" name="venue" id="venue" data-options="required:true" />
                <input class="easyui-combobox" style="width:245px; height:32px" name="venroom" id="venroom" data-options="required:true" />
            </div>
        </div>--%>
    ...
    <div class="whgff-row none nobm">
        ...
        <div class="whgff-row-input">
            <input class="easyui-combobox" id="groupid" name="groupid" value="${whgTra.groupid}" style="width:500px; height:32px" data-options="limitToList:true, valueField:'id', textField:'name'"/>
        </div>
    </div>
    <div class="whgff-row">
...
<script>
    ...
    function myChangeProvince(newVal, oldVal) {
        changeProvince(newVal, oldVal, function(){
            if(typeof(window.__init_city) == 'undefined'){
                $('#__CITY_ELE').combobox('setValue', '${whgTra.city}');
    ...
    $(function () {
        WhgComm.initPMS({
            ...
            venEid:'venue', venValue:'${whgTra.venue}',
            roomEid:'venroom', roomValue:'${whgTra.venroom}'
        });

        //初始省值
        $('#province').combobox('setValue', '${whgTra.province}');
        //$("#ekey").combobox("setValue","${whgTra.ekey}");

        var infostate = '${whgTra.state}';
        if (infostate && infostate == "4") {
            showXjReason('${whgTra.id}');
        }
        //根据地址取坐标
            ...
            if(length >= 1){
                for(var i=0;i<length;i++){
                    if(i<1){
                        ...
                        content += '<div class="whgff-row multi_edit js-temp-addnow" name="multiContent">' + addHtmlContent + '</div>';
                    }
                }
        ...
        $('.whg-js-data').on('click','input[name="ismultisite"]',function(){
            if('${whgTra.state}' == '4' || '${whgTra.state}' == '6'){//上架上架时不能修改
                return false;
            }
            var ck = this.checked;
            var value = $(this).val();
        ...
        //定义表单提交
        var url = "${basePath}/admin/traLive/edit";
        frm.form({
            url: url,
            novalidate: true,
