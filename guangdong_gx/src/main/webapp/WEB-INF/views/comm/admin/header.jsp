...
<script type="text/javascript">
    ...
    $.ajax({
        ...
        success: function(data){
            if (data) {
                managerCults = data.sessionAdminUserCults;
                managerDepts = data.sessionAdminUserDepts;
            }
        }
    });
    function showXjReason(fkid, state, frmjq) { //下架原因
        if (!fkid || fkid=='') return;
        if (state && state!=4) return;
        if (frmjq && !frmjq.find("#showReason").length){
            var tempHtml = '<div class="whgff-row"  id="showReason">\
                <div class="whgff-row-label"> 下架原因： </div>\
                <div class="whgff-row-input">\
                    <input class="easyui-textbox" id="reason" readonly="true" value="" multiline="true" style="width:350px;height: 150px;">\
                </div>\
            </div>';
            frmjq.append(tempHtml);
            $.parser.parse($("#showReason"));
            $("#showReason").hide();
        }

        var url = getFullUrl('/admin/reason/getReason?fkid=' + fkid);
        $.ajax({
            url: url,
            type: "POST",
            success: function (data) {
                if (data.success && data.success != 0 && data.data && data.data.reason && data.data.reason != "") {
                    $("#showReason").show();
                    $("#reason").textbox("setValue", data.data.reason);
                }
            }
        });
    }
...
<style type="text/css">
    ...
    .cultIcon{
        background: url(${basePath}/static/admin/img/cult.png);
    }
</style>