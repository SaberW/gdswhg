package hn.creatoo.com.web.actions.supply.login;
...
public class GxLoginAction {
     ...
     * @return
     */
    @RequestMapping("/res-gx/login")
    public ModelAndView toLogin(HttpServletRequest request, HttpServletResponse response) {
        ModelAndView view = new ModelAndView();
        String url = request.getHeader("Referer");
        view.addObject("temp_url",url);
        view.setViewName("guangdong_gx/login/login");
        return view;
    }
    ...
    public ModelAndView doLogin(HttpServletRequest request, HttpServletResponse response,RedirectAttributes model) {
        ...
        try {
            ...
            //调用登录接口
            String a = RestUtils.post(restTemplate, "api/inside/validLogin", params);//restTemplate.postForObject("http://localhost:8080/api/doLogin", reqJsonStr, String.class);
...
//                System.out.println("====================data:" + json.get("data").toString() + "============================");
                //登录成功设置会话
                WhgSysUser user = JsonUtils.toJavaObj(json.get("data").toString(), WhgSysUser.class);
                request.getSession().setAttribute(SysLoginInterceptor.sysSessionUserKey, user);

                //跳转到之前的页面
                String url = request.getParameter("history_url");
                if(url == null || StringUtils.isEmpty(url) || url.endsWith("login")){
                    if(request.getServerPort() == 80){
                        url = request.getScheme() + "://" + request.getServerName() + request.getContextPath()+"/res-gx/index";
                    }else{
                        url = request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort() + request.getContextPath()+"/res-gx/index";
                    }
                }
                ...
                view.setViewName("redirect:/res-gx/login");
            }
        } catch (Exception e) {
     ...
     * @return
     */
    @RequestMapping("/res-gx/doLogout")
    public ModelAndView doLogout(HttpServletRequest request, HttpServletResponse response) {
        ModelAndView view = new ModelAndView();
        try {
            request.getSession().removeAttribute(SysLoginInterceptor.sysSessionUserKey);
            view.setViewName("redirect:/res-gx/login");//登录界面
        } catch (Exception e) {
            log.error(e.getMessage(), e);
        }
        return view;
