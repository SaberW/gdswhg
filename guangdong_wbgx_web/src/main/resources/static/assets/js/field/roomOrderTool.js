...
RoomOrderTool.prototype = {
    ...
    buttonSubmit: function(){
        ...
        if (this.settings.venueOrderCheckURL && this.settings.userId && this.settings.userId != ''){
            var params = {
                roomTimeId: this.selectValue.id,
                userId : this.settings.userId
            };
            var vocUrl = this.settings.venueOrderCheckURL;

            var basePathHeader = this.settings.basePathHeader,
                roomTimeId = this.selectValue.id,
                timeday = this.selectValue.timeday,
                timestart = this.selectValue.timestart,
                timeend = this.selectValue.timeend,
            ...
            dataInit.ajax({
                ...
                fn: function(data){
                    if(data.code == 0 ) {
                        window.location.href = basePathHeader + '/cg/venueliucheng?roomtimeid=' + roomTimeId + '&timeday=' + timeday + '&timestart=' + timestart + '-' + timeend + '&roomId=' + roomId;
                    }else if (data.code == 200){
                        rongDialog.init({
                            type: 2,
                            desc: data.msg || '您还未完成实名认证，请先完成实名认证！',
                            nextFn: function(){
                                window.location.href = basePathHeader+'/center/safely/real';
                            }
                        });
                    }else{
