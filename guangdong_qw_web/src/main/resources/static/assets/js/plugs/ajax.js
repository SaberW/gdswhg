...
var dataInit = {
    ...
    getImage: function(url, width, height){
        var imgurl = url.replace(/(\.\w*)$/, "_"+width+"_"+height+"$1");
        return imgurl;
    },
    getSmallImage: function (url) {
        ...
        if(url){
            ...
            for(var i=0; i<img.length; i++){
                if(i != 0){ imgurl += '.'; }
                imgurl += (i == img.length-2) ? (img[i]+'_700_350') :(img[i]);
            }
        }
        return imgurl;
    },
    getMidImage: function (url) {
        ...
        if(url){
            ...
            for(var i=0; i<img.length; i++){
                if(i != 0){ imgurl += '.'; }
                imgurl += (i == img.length-2) ? (img[i]+'_750_500') :(img[i]);
            }
        }
        return imgurl;
    },
    getBigImage: function (url) {
        ...
        if(url){
            ...
            for(var i=0; i<img.length; i++){
                if(i != 0){ imgurl += '.'; }
                imgurl += (i == img.length-2) ? (img[i]+'_720_540') :(img[i]);
            }
        }
        return imgurl;
    },
    ...
    share: function (title, imgPath) {
        ...
        if ($('a.fxweix').size() > 0) {
            ...
            $.getScript('http://static.bshare.cn/b/buttonLite.js#ssc=false', function(){
                bShare.init();
                $('a.fxweix').on("click", function(){
                    bShare.share(event,'weixin');
                    return false;
                })
            })
        }
