package hn.creatoo.com.web.actions.train.pxsz;
...
/**
 * Created by zengrong on 2017/09/27.
 */
@Controller
@RequestMapping(value = "/{cultsite}/pxsz")
public class PxszAction {

    //包装页面头部SEO信息
    private void setCommcontent(ModelAndView mv,String title,String desc,String key){
        String pageTitle = "文化在线-广东公共数字文化联盟-培训师资 ";
    ...
    public ModelAndView toList(HttpServletRequest request,@PathVariable("cultsite") String cultsite){
        ...
        modelAndView.setViewName("guangdong_gx/pxsz/list");
        request.setAttribute("qh",true);
        this.setCommcontent(modelAndView,"师资列表",null,null);
        return modelAndView;
    }
    ...
    public ModelAndView toDetail(HttpServletRequest request,@PathVariable("cultsite") String cultsite){
        request.setAttribute("cultsite",cultsite);
        ModelAndView modelAndView = new ModelAndView();
        modelAndView.setViewName("guangdong_gx/pxsz/detail");
        this.setCommcontent(modelAndView,"师资详情",null,null);
        return modelAndView;
    }
}
