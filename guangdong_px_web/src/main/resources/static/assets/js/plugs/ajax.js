...
var dataInit = {
    ...
    getSmallImage: function (url) {
        var img = url.split('.');
        if (img.length > 1)
            return url.substring(0,url.indexOf('.'+img[img.length-1])) + '_300_200.' + img[img.length-1];
        return url;
    },
    ...
    getBigImage: function (url) {
        var img = url.split('.');
        if (img.length > 1)
            return url.substring(0,url.indexOf('.'+img[img.length-1])) + '_740_555.' + img[img.length-1];
        return url;
    }, getRemoteURL: function (path,url) {
